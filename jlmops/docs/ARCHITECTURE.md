...
#### 2.5.4.1. Concurrency Safety (locked reads/writes)

The Daily Sync widget polls every 1 second during spinner stages, so `system.sync.state` and `SysJobQueue` rows are read and written by many concurrent Apps Script executions in practice, not just sequential ones. Two locked primitive pairs close this, both built on `LockHelpers.js#withScriptLock` (a single script-wide `LockService` mutex — not reentrant, not keyed per-resource):

*   **Sync state:** `SyncStateService.mutateSyncState(fn)` / `mutateSyncStateBestEffort(fn)` — `fn` receives the fresh, locked state, mutates it in place, and either returns (apply) or throws `SyncStageStaleError` (guard no longer holds — e.g. a concurrent execution already advanced the stage). The throwing variant is for a function's first, user-initiated write (contention/staleness surfaces as `SyncLockContentionError`/`SyncStageStaleError` to the caller); the best-effort variant is for every write after that, and for idempotent background pollers (`OrchestratorService._checkAndAdvanceSyncState`'s three branches), where losing the race is a normal, silent no-op, not a failure. `transition()`, `updateStep()`, and `resetSyncState()` are built on these, so no caller needs to lock directly.
*   **Job queue:** `OrchestratorService.setJobRowStatus(sheet, headers, jobId, fn)` / `setJobRowStatusBestEffort(...)` — the same shape, keyed by `job_id` rather than a remembered row number (a job's row can shift after `HousekeepingService.purgeOldJobs` removes old rows (its read-clear-rewrite now runs inside `withScriptLock` itself, re-reading `SysJobQueue` fresh inside the lock — the row numbers it produces are still not stable across time), so anything reading or writing a specific job's row after claim time goes through `_getJobRowByJobId`/`_getJobRowByJobIdWithRetry`, never a captured row index). `_claimNextPendingJob` (+ its bounded-retry wrapper) is the locked equivalent for picking up the next `PENDING` job. Every job-processor service's `_updateJobStatus` (`ProductImportService.js`, `OrderService.js`, `WooInventoryPushService.js`, `ValidationOrchestratorService.js`) returns whether its write actually applied; callers gate `finalizeJobCompletion`/success logging on that instead of proceeding unconditionally.
*   **General config writes:** `ConfigService.setConfigLocked(settingName, key, value)` wraps the otherwise-unlocked `setConfig` for general-purpose callers. Two call sites are deliberately excluded and must keep calling the raw `setConfig` directly: `SyncStateService.setSyncState` (already runs inside the sync-state lock above) and `rebuildSysConfigFromSource`'s `RUNTIME_KEYS` restore loop (locks each key itself, comparing the live value against its known post-clear default before restoring). Nesting `setConfigLocked` inside either would hang for the lock's timeout and then throw, on every write.

None of these locks are reentrant or composable across the two pairs — code holding one must never call into a path that acquires the other, or acquires its own kind again, in the same call stack.

#### 2.5.4.2. Sync-Active Guard (background work vs. an in-progress sync)

`SyncStateService.isSyncActive()` (stage is set and not `IDLE`/`COMPLETE`/`FAILED`) is the single shared guard against housekeeping or product-pull work running concurrently with a user-driven sync session, replacing hand-copied stage-list checks that had drifted out of sync in three separate places. Applied individually, not as a whole-function gate, to `HousekeepingService.archiveCompletedOrders`/`archiveCompletedTasks`/`purgeOldJobs` and to `WooProductPullService.pullBundleProducts`/`pullProducts`. Building this guard found a third, previously undocumented broken one: `pullProducts` (the main product-pull entry point) had been checking a nonexistent `syncState.currentStage` field instead of `.stage`, so it had silently never fired since it was written.

`HousekeepingService.performFrequentMaintenance`'s own top-of-function `isSyncActive()` check can go stale by the time execution reaches its `pullOrders()` call (business-hour math plus a config read happen in between). Rather than lock the whole ~60s+ pull — which would starve `doGet`/`doPost` — it re-checks `isSyncActive()` a second time, fresh, inside a short `withScriptLock` hold, immediately before calling `pullOrders()`, aborting the cycle if a sync started in the gap, then releases the lock before the pull itself runs. A sync starting in the instant between lock release and the pull's first write is an accepted residual risk, the same tradeoff the job-queue locking above already makes for its own inner-service calls.
...
*   **Trigger installation model (operational caveat):** No triggers are installed by code — all recurring triggers (the hourly maintenance/zombie-killer pass, the frequent poll, and the daily housekeeping) are created **manually in the Apps Script Triggers UI**. Consequence: a deleted recurring trigger is **not** self-reinstalled, and its absence is invisible in code review. Re-create it in the Triggers UI if a scheduled pass stops firing. (Bundle health was previously also auto-checked via a code-installed one-off post-sync trigger plus a daily-maintenance step; both removed 2026-09-02 — bundle deficiency review is now manual-only, triggered by the user from the Bundles view, since the cached result was frequently stale by the time it was seen.)
