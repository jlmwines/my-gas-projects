...
### `SysCategories` (category authority — consolidated 2026-07-21)
*   **`SysCategories`** (`sct_Code, sct_NameEn, sct_NameHe, sct_ComaxDiv, sct_ComaxGrp, sct_Value` — `config/schemas.json`, key column `sct_Code`): the authoritative category reference table. Maps a WC-style category slug (`sct_Code`, e.g. `dry-red`) to EN/HE names, Comax division/group, and `sct_Value` (the WooCommerce category term ID, e.g. `903` for Gift Items — populated via a one-time `/wc/v3/products/categories` pull, matched by `sct_ComaxGrp`/`sct_ComaxDiv` for Comax-mapped rows or by name for the rest). Owner-curated and live-current as of 2026-07-21 (`CONFIG_COMPLIANCE_PLAN.md` Phases 1–2).
*   **Consolidation complete (`CONFIG_COMPLIANCE_PLAN.md` Phases 3–4, 2026-07-21):** `SysCategories` replaced the parallel category logic that used to live in `SysLkp_Texts`' `Category`/`ComaxCat`-note rows and in three separate hardcoded JS maps. Current readers: `ContactEnrichmentService.js` (`_loadCategoriesLookup`, keyed three ways — `sct_NameEn.toLowerCase()`, `sct_ComaxGrp` for wine, `sct_ComaxDiv` for non-wine — feeds CRM preference tagging), `ConfigService.getCategoryDivisionMap()` (Hebrew-name → single division code, used by `HousekeepingService.js`'s stock-deficiency check and `WebAppProducts.js`'s product suggestion/search), and `ConfigService.getCategoryTextLookup()` (keyed by `sct_ComaxGrp`, feeds `WooCommerceFormatter.js`'s `getLookupText('categories', ...)` for the category line in product descriptions, consumed by `ProductService.js`'s export/preview/backfill functions and `WebAppProducts.js`'s live preview).
*   **`SysLkp_Texts`' `Category`/`ComaxCat`-note rows are superseded, pending removal.** Nothing reads them anymore as of the Phase 4 repoint (deployed @518, live-verified). Owner is holding the actual row deletion for a short production soak period before cleanup — see `plans/CONFIG_COMPLIANCE_PLAN.md` for the exact 10 rows. `SysLkp_Texts`' `WebCat`-note rows (a separate subset, `slt_Code = 'WebCat'`, feeds the Bundles-UI category dropdown in `WebAppBundles.js`) are unaffected and still live — not part of this consolidation.
*   **`SysLkp_Texts` itself** (`slt_Code, slt_TextEN, slt_TextHE, slt_Note` — `schema.data.SysLkp_Texts` registered 2026-07-21, though still read dynamically via `LookupService.getLookupMap('map.text_lookups')`, `LookupService.js:14-60`) remains the live table for everything else it always held — tasting-attribute label/circle codes, pairing/kashrut text, promo text, URLs. `SysLkp_Grapes`/`SysLkp_Kashrut` are the same pattern, also registered.
