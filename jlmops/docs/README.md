...
- **Length trims (mostly done).** Done 2026-06-11: `UI_AUDIT` 821→51, `CAMPAIGN_SYSTEM_PLAN` 1026→77, `CRM_PLAN` 679→70, `CONTENT_LIBRARY_PLAN` 1210→142 (all cut to current-state design + open intent; shipped specs, dated decision-history, and graduated schema removed — those live in git/session-log + the system docs). `RELIABILITY_AUDIT` header reframed. **Still to trim:** `RELIABILITY_AUDIT` (935 — collapse the interleaved per-session SHIPPED Status blocks + cut §9/§10 review history; keep the open session queue). One focused pass left.
