...
**Status:** In progress — **Stage 0 complete (@227 calc + @230 write paths)** and **Stage 1 shipped (@228→@230)**, both verified live; Stages 2–7 planning. **Stage 2 decisions finalized 2026-06-06** (ex-VAT/18% formula; cost stored on CmxProdM, rate cached on WebProdM; no auto-default; literal-0 sentinel rule dropped — see Stage 2). Staged — each stage is independently shippable; **decide depth as we refine**. No code until per-stage user go.
...
- **Missing cost → manual backfill, no default.** A default would poison the ranking signal; missing must read as *unknown*, never as a plausible value. Backfilled rates survive re-import (the import only overwrites products it has cost for). Gaps are a **legacy one-time cleanup**, not a recurring fill (new Comax products carry reliable cost), so manual backfill is cheap; the auto-default (25%) idea was considered and **dropped 2026-06-06**. No sentinel/literal-0 rule is needed — there are no genuine 0% rates and no 0-cost products, so blank simply means *missing*.
...
- **Missing cost → manual backfill (no default).** Surface a "products missing cost" list (parity-validator pattern) off blank `cpm_Cost` + an inline **"—/no cost"** marker; never render as `0%`; exclude such products from the profit-sort (or pin to a flagged group). Gaps are a **legacy one-time cleanup** (new Comax products carry reliable cost); a backfilled cost persists because the import only overwrites products it has cost for.
