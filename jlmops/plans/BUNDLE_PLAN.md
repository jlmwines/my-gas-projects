...
- **Store the profit RATE, not cost.** Compute `(wpm_RegularPrice − cost) / wpm_RegularPrice` and write append-only `cpm_ProfitRate` on `CmxProdM`. Cost transient. Rate is the per-product lever and varies widely across the catalog. **Cross-sheet join:** cost comes from the cost file (matched into `CmxProdM` by SKU), but the **selling price is `wpm_RegularPrice` from WebProdM** — so the import joins CmxProdM↔WebProdM by SKU at compute time. A Comax product **not on the web** (no WebProdM row) has no computable rate → treat as missing (same as empty cost).
