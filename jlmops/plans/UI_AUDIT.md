...
- **T4.3 ManagerInventory mobile pass — SHIPPED 2026-05-29 (Stage A @168/172, B @170/174, C @172/176).** A: card-headers `flex-wrap` + Brurya add-group full-width on mobile. B: Product Counts collapses to a compact list on mobile; tap-row → focused count-entry modal (Storage/Office/Shop/Vintage/Comment + Save / Save & next) via shared `ModalOverlay` (added TaskWidgets include to this view). C: Brurya table → **custom `.brurya-table` 2-row mobile card** (Quantity+SKU / Name; # hidden) per user, not generic responsive-stack. **Stage B shipped-but-UNSMOKED** (no count tasks scheduled at ship time — verify modal flow when they appear); A + C user-confirmed on mobile. Mid-session also fixed a latent **T1.0** bug: Brurya autocomplete CacheService 100KB overflow ("Argument too large") @169 deploy @173.

- **T4.1 OrdersView mobile — SHIPPED 2026-05-29 (@173 deploy @177 + @174 deploy @178, `7e1f643`).** Both tables → `.responsive-stack` + `table-sm` + `data-label` (labelled cards on mobile); mobile-safe "Select all" button (thead checkbox hidden by responsive-stack on mobile); packing-slip checkbox pinned 22×22 + relabelled "Print this order" after smoke. First production adopter of `.responsive-stack`. User-confirmed on mobile.

- **T4.4 ManagerProductsView mobile — SHIPPED 2026-05-29 (@178 deploy @182 + @179 deploy @183, `e97df74`).** Modal mobile: comparison-grid single-column (+`min-width:0` to defeat grid overflow), tab bar scrolls with Fill/Clear on a row below, header stacks (RTL Hebrew name no longer truncated-from-start). Both outer tables → responsive-stack. **Shared fix:** AppView responsive-stack now suppresses the bare ": " on no-`data-label` cells (also fixed OrdersView). User-confirmed on mobile.
- **ManagerDashboard simplification — SHIPPED 2026-05-29 (@175→@177 deploy @179→@181).** Tasks list-only (calendar toggle + week-nav hidden — were overflowing off-screen); compact mobile task cards (Title own line + meta line). User-flagged. Calendar markup/JS now dead (later cleanup).

- **T4.5 LibraryView mobile — SHIPPED 2026-05-29 (@180 deploy @184 + @181 deploy @185, `18a2cd7`).** Drawer header single-column; task-row Status label; library list table → responsive-stack; both tabs' mobile cards unified to one compact flat style with a clear divider (scoped to `.lv-*`). User-confirmed. **Tier 4 mobile COMPLETE (T4.1–T4.5).**

- **T5.1 AdminContacts modal cleanup — SHIPPED 2026-05-29 (@182 deploy @186; nav @183/@184 deploy @187/@188).** 3 Bootstrap modals → modal-overlay via the shared **ModalOverlay** helper (focus-trap/Esc/scroll-lock — improvement over the plan's raw display toggle); 2 `btn-primary` → `btn`. Verified 0 modal-fade/$().modal()/btn-primary. **Nav fix:** admin "Contacts" was routing to the manager view; now a single admin "Contacts" → AdminContacts (consistent with manager nav). **Surfaced a design rough-spot → new plan `CONTACT_ACTION_RIBBON_PLAN.md`** (unified packaged "Make contact" + record ribbon; will rework these same modals later — T5.1 is groundwork).

§3 surfaces updated: §3.3 (orders merged), §3.12 (SystemHealthView retired), §3.13 (orphan widgets retired). **Recommended remaining order:** T5.2 (AdminProducts modals → ModalOverlay, same recipe as T5.1), T5.3 (shared-list-component decision, conditional). **Tier 4 mobile COMPLETE.** `.responsive-stack` in 3 views + utility; ModalOverlay now consumed by ManagerContact (CCP-UI-8) + AdminContacts (T5.1). **New queued feature:** `CONTACT_ACTION_RIBBON_PLAN.md` (contact-outreach packs). T2.1 (bundles) deprioritized per user (desktop-only). **T5.3 shared-list-component decision reachable now** (7 kit consumers — audit filter/sort/render commonality; the consolidated-`getViewData` + optional-preload-loader pattern is now established in 3 views: AdminProducts (T3.1), ManagerContact (T3.2 `renderContactList`), AdminBundles (T3.3)). T2.1 (bundles) deprioritized per user (desktop-only). **T5.3 shared-list-component decision reachable now** (7 kit consumers — audit filter/sort/render commonality; the consolidated-`getViewData` + optional-preload-loader pattern is now established in 3 views: AdminProducts (T3.1), ManagerContact (T3.2 `renderContactList`), AdminBundles (T3.3)).
