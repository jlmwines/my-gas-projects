...
- **T3.1 AdminProducts refreshView consolidation — STAGE A LANDED 2026-05-29 (NOT deployed); Stage B = next session.** Backend `WebAppProducts_getAdminViewData` added + `clasp push`ed (editor-tested clean), but NOT in live @162 — nothing calls it until Stage B, so live is unaffected. ⚠️ v1 deep-dive code assumptions were wrong (getters are global, return raw arrays; lookups `{error,data}`) — **deep-dive corrected**; Stage B must follow the corrected shapes. Stage B = refactor 9 loaders + `refreshView` to single round-trip on AdminProductsView (the critical product-accept flow; splice path must stay intact).

§3 surfaces updated: §3.3 (orders merged), §3.12 (SystemHealthView retired), §3.13 (orphan widgets retired). **Recommended remaining order:** T3.1 (AdminProducts refreshView) → T3.2 (ManagerContact load-once) → T3.3 → [CCP-UI-8 helper] → T4.x → T5.x. T2.1 (bundles) deprioritized per user (desktop-only). **T5.3 shared-list-component decision reachable now** (7 kit consumers — audit filter/sort/render commonality).
