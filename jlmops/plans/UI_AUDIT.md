...
§3 surfaces updated: §3.3 (orders merged), §3.12 (SystemHealthView retired), §3.13 (orphan widgets retired). **Recommended remaining order:** T3.2 (ManagerContact load-once) → T3.3 (AdminBundles, same recipe) → [CCP-UI-8 helper] → T4.x → T5.x. T2.1 (bundles) deprioritized per user (desktop-only). **T5.3 shared-list-component decision reachable now** (7 kit consumers — audit filter/sort/render commonality; account for the per-loader optional-preload pattern T3.1 established).
