...
§3 surfaces updated: §3.3 (orders merged), §3.12 (SystemHealthView retired), §3.13 (orphan widgets retired). **Recommended remaining order:** [CCP-UI-8 ModalOverlay helper — prerequisite] → T4.x → T5.x. T2.1 (bundles) deprioritized per user (desktop-only). **T5.3 shared-list-component decision reachable now** (7 kit consumers — audit filter/sort/render commonality; the consolidated-`getViewData` + optional-preload-loader pattern is now established in 3 views: AdminProducts (T3.1), ManagerContact (T3.2 `renderContactList`), AdminBundles (T3.3)).
