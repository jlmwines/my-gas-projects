...
- **T2.4 — Content Stream modal merge (resolved 2026-07-08).** A third+ consumer emerged (`AdminTasksView`/`LibraryView`/`PublishingView` each had their own copy) and `CALENDAR_LIBRARY_LOOP_PLAN.md` Phase 2 built the shared `ContentStreamModal.html` for those three. `AdminProjectsView` was excluded — it's abandoned (no nav entry), not an active consumer. Deep-dive (archived): `_archive/UI_T2_4_content_stream_modal.md`.
