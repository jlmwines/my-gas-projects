...
§3 surfaces updated: §3.3 (orders merged), §3.12 (SystemHealthView retired), §3.13 (orphan widgets retired). **Recommended remaining order:** T4.5 (Library mobile) → T5.x. **Tier 4 mobile complete except T4.5** (T4.1+T4.2+T4.3+T4.4 done). `.responsive-stack` now adopted in 3 views (Orders, ManagerProducts, + utility); ModalOverlay available for T5.1/T5.2. T2.1 (bundles) deprioritized per user (desktop-only). **T5.3 shared-list-component decision reachable now** (7 kit consumers — audit filter/sort/render commonality; the consolidated-`getViewData` + optional-preload-loader pattern is now established in 3 views: AdminProducts (T3.1), ManagerContact (T3.2 `renderContactList`), AdminBundles (T3.3)). T2.1 (bundles) deprioritized per user (desktop-only). **T5.3 shared-list-component decision reachable now** (7 kit consumers — audit filter/sort/render commonality; the consolidated-`getViewData` + optional-preload-loader pattern is now established in 3 views: AdminProducts (T3.1), ManagerContact (T3.2 `renderContactList`), AdminBundles (T3.3)).
