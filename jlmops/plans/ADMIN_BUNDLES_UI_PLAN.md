...
- **Margin caveat — RESOLVED @256.** `BundleService._calculateBundlePrice` previously read `sb_DiscountPrice` (vestigial, blank on import), ignoring the real discount. Now it reads the bundle's own WOOSB row (keyed by `wpm_ID` = bundleId) via `_buildBundleDiscountMap` + `_bundleDiscountFromWeb`: precedence (1) `wpm_WoosbDisableAutoPrice` → fixed `wpm_WoosbCustomPrice`; (2) `wpm_WoosbDiscount` on → `wpm_WoosbDiscountAmount` off the member sum, `'%'`-suffixed = percentage else fixed amount (**user-confirmed fixed-amount 2026-06-07**); (3) else no discount. Falls back to `sb_DiscountPrice` only if no web row found. Applied in both `getBundleWithSlots` (editor) and `getAllBundles` (list). Read-only/math — never pushed back.
