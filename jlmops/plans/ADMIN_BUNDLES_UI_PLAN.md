...
**Owner:** Dispatch-led (UI/UX redesign of `AdminBundlesView`). Main CLI session owns the data/serializer/export layer in `_archive/BUNDLE_PLAN.md`.
**Relationship:** This doc owns the **editor UI/UX** at a deeper level than `_archive/BUNDLE_PLAN.md`'s "Workspace = the existing editor, polish as needed" (Stage 3). When the two overlap, the data contract lives in `_archive/BUNDLE_PLAN.md`; the surface/layout lives here.
...
**Out of scope (owned by `_archive/BUNDLE_PLAN.md`):**
...
- These provide the data contracts the UI renders; change requests against them go to `_archive/BUNDLE_PLAN.md`.
...
- **Health alerts panel** (`#health-alerts-container`) — low-inventory replacement suggestions; this is the **"unused alert view"** that `_archive/BUNDLE_PLAN.md` Stage 7 plans to replace with a proactive suggestion view. Coordinate with that stage.
...
From `_archive/BUNDLE_PLAN.md` (these are already specced at the data level; the redesign should incorporate their surfaces):
...
- **Optional / qty-variable authoring toggle** — so ops can author the `optional` flag per slot (Stage 3; the field is now re-derived from web each refresh — see `_archive/BUNDLE_PLAN.md` Stage 3 fix 2026-06-07 — so an editor toggle is the authoring path, exported same session).
...
Today every slot edit commits immediately and deletes are one confirm from gone. Instead: edits mutate a **client-side draft**; deletes render struck-through with Undo; one **Save Composition** commits atomically via a **new backend `WebAppBundles_saveComposition(bundleId, header, slots[])`** (contract owned by `_archive/BUNDLE_PLAN.md`), with `TaskWidgets.confirm` listing deletions. Draft mirrors to `sessionStorage` per `RELOAD_RESILIENCE_PLAN.md` A0 + `beforeunload` desktop seatbelt. After save, the row shows "differs from web — export pending" (which is simply true, via the diff).

### 7.6 Export UX
Export button: run the fresh diff + the targeted out-of-stock failsafe (`_archive/BUNDLE_PLAN.md` §3.1) → **generate the CSV through the product-detail export sequence and auto-open it as a new sheet** (user decision §6) — one row per bundle, EN and HE woosb cells side by side. File production **auto-closes** `task.bundles.push_pending`. Optional nicety: in-sheet/in-session strikethrough tracking only — no persisted "pasted" state.
...
- **Phase 4 — Stage 7 hook. ✅ SHIPPED @275→@282.** Stage 7 generator landed (`_archive/BUNDLE_PLAN.md`), so this surface grew to drive + explain it. Toolbar relabel/reorder: **Review / Maintain / Export / Pull from Web** (primary) + muted **Update Composition** utility; all major buttons confirm-gated (Review is slow + accidentally triggerable). **Two distinct per-bundle signals** surfaced and kept separate: **Needs attention** (deficiency, from `getBundleDeficiencies`) vs **Update web** (ops≠web export diff, from the push-status cache) — uncolored Flags column + chips (All / Needs attention / Update web / OK); Status column kept (no color). push_status is a recomputed cache → refreshed on save/maintain/reroll/pull + frontend `syncPushStatus` (fixed the stale "Matches web" — it was never an overnight revert). Perf: shared eligible-products `ctx` (no per-slot sheet reads). **@282 editor reveals WHY** — `getBundleDeficiency` (per-bundle) → deficiency strip summary + per-slot warning tags (out/low stock, off-criteria, empty, base total ∉ band).

- **Phase 5 — Inline editor + per-bundle export. ✅ SHIPPED 2026-06-09 @284→@286 (live-smoked OK).** Post-deploy fixes during smoke: **@285** removed the `max-height:300px` list-wrapper cap that clipped the inline editor (the editor now mounts as a row *inside* that table); **@286** mobile pass (≤768px) — card-header titles stack above their button bars, buttons go full-width stacked with separation, filter chips gapped + name filter full-width (CSS-only: one media query + `bundle-card-header`/`bundle-btnbar` marker classes; desktop unchanged). Three stages: **A — inline-at-row editor:** the editor is one persistent node that mounts into an expansion `<tr>` under the active bundle row and parks back home (hidden) on close or before any list re-render, so `loadBundleList()`'s `tbody.innerHTML` (fired by save/Maintain/Review) can't destroy it; accordion toggle (click open row to close). **B — per-bundle export inside the row:** the EN/HE export-meta copy panel now renders in `#editor-export-meta` *inside* the editor card (toggle button, self-clears on load/edit/full re-render) — it parks and closes with the row, retiring the orphaned-panel bug; the shared bottom panel is now reserved for GLOBAL batch results (Maintain/Review/Export/Update) and is no longer wiped on close or bundle-switch. **C — single-bundle Maintain + Update-from-web:** new editor-header buttons → `BundleService.maintainBundle(bundleId)` / `WebAppBundles_maintainBundle` (mirrors the batch Maintain, scoped to one) and `WebAppBundles_updateComposition(bundleId)` via a filtered `WebAppBundles_reimportAllBundles(bundleId)`; Re-roll was already single-bundle. The original design shape (kept for reference):
  - **Inline-at-row editor with export attached inside it.** Per-bundle export becomes the primary path (the row carries its own EN/HE woosb meta + copy panel); the batch Google Sheet (Phase 2) stays as the fallback for bulk work.
  - **Single-bundle target for "Update Composition"** — the muted utility runs against one bundle in context, not only the global batch.
  - **Resolves the orphaned export-meta panel bug (2026-06-09).** Today the EN/HE "Export meta" copy panel renders into the shared `action-results-panel` (a sibling of the editor); `closeEditor()` hides the editor but not that panel, so a closed bundle's woosb text lingers with no visual owner → wrong-bundle copy/paste risk. Phase 5's inline-at-row editor (export panel lives *inside* the row) eliminates this class of bug by construction — collapsing the row takes the panel with it. Interim one-line fix available now if needed (`hideActionResults()` in `closeEditor()`, mirroring the existing bundle-switch/save calls); deferred with this phase per user.
  - **Operating sequence (the mental model behind the toolbar):** Refresh (daily sync — Comax is inventory truth) → Review → Correct (Maintain / Re-roll / manual edit) → Export (paste woosb JSON into WPClever) → Confirm (Pull from Web closes the ops≠web diff). Pull from Web only matters *between* syncs (full sync already captures woosb_ids into master).
  - **Companion backlog (separate homes):** bundles missed-profit report (jlmops wishlist); website woosb bundle-explainer dismissible message, EN+HE (theme wishlist).
  - **Sizing (2026-06-09, grounded in current code).** **~1–1.5 build sessions, frontend-concentrated, low data-layer risk** — almost everything underneath already exists (composition sheet, draft/save, `showExportMeta`, deficiency strip, export diff, per-bundle generator). Breakdown:
    - **A. Editor mount relocation — the bulk (~0.5–1 session).** Today the editor is ONE shared `bundle-editor-section` with singleton element IDs (`composition-list`, `editor-deficiency-strip`, `hdr-*`), populated by `loadBundleForEditing(id)` on row click (`AdminBundlesView.html:340-402`). Inline-at-row = inject that editor as an expansion block *under the active row* instead. **Keep it a single editor instance** mounted under the selected row (recommended) — all the `render*` functions and their fixed IDs are reused unchanged; only the mount point + collapse/teardown rewire. Going to *true per-row editors* (multiple open at once, unique IDs) is the heavier path and would add ~+0.5 session — avoid unless the user wants simultaneous open rows.
    - **B. Per-bundle export inside the row (~2–3 hrs).** Move `showExportMeta` output from the shared `action-results-panel` into the inline block (the EN/HE woosb generation already exists). Retires the orphaned-panel bug by construction.
    - **C. Single-bundle Maintain / Update Composition (~2–3 hrs).** Thin backend wrappers: `_generateBundleComposition(bundle, opts)` is already per-bundle and `rerollBundles(bundleId)` already single-bundle — add a `maintainBundle(bundleId)` mirror + a single-bundle `updateComposition(bundleId)` path + their `WebAppBundles_*` controllers, then a per-row button. No new generation logic.
    - **D. Collapse/teardown + per-row draft seatbelt (~1–2 hrs).** Mostly exists — `draftKey(bundleId)` + `sessionStorage` + `closeEditor()` already key drafts per bundle.
    - **E. Smoke (within session).** Inline editor edit→save, per-bundle EN/HE paste, single-bundle Maintain.
    - **Risk/unknown:** the single-vs-multi open-row decision (above) is the one fork that moves the estimate; everything else is reuse. No serializer / data-model / new-task work.

## 10. New pieces required
- `WebAppBundles_saveComposition(bundleId, header, slots[])` — atomic batch commit (data contract → `_archive/BUNDLE_PLAN.md`).
- Housekeeping `needsPush` computation + cache (data contract → `_archive/BUNDLE_PLAN.md`).
...
- Export CSV exact column shape (EN/HE side-by-side confirmed as intent; final contract per `_archive/BUNDLE_PLAN.md` Stage 3).
