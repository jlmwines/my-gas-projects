...
5. ✅ **SHIPPED 2026-07-14 @480. A second, distinct race found smoke-testing @479 live: load-vs-load, not submit-vs-load.** Manager reported: verification submit ~10s + ~5s list reload, then opening the *next* verification showed stale data in the modal; separately, a new-product onboarding modal showed "current data" that should have been blank (no master/staging row exists yet for a genuinely new SKU). Traced `WebAppProducts_loadProductEditorData` server-side — confirmed no server bug, it correctly returns empty `master`/`staging` for a SKU with no `WebDetM`/`WebDetS` row. The actual cause: `openEditor()`'s own data-fetch call (`ManagerProductsView.html:1107-1154`, separate from the @479 submit guard) had no protection against being called a second time before the first call's response arrived — whichever response landed *last* won, regardless of which task was actually being viewed by then. **Fix:** a `_loadToken` counter, incremented on every `openEditor()` call and captured locally; both the success and failure handlers discard themselves if their captured token no longer matches the live one (a newer `openEditor()` has since started). Not yet smoke-tested live.
