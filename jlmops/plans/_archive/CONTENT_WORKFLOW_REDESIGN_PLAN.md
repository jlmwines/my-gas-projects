...
**Decided and shipped 2026-06-18. Shipped @316–@322; see implementation status below.**
...
**Implementation status (2026-06-18, shipped @316–@318).** Built via the user's simpler **attach-to-replace** model rather than a fork engine: a new version is produced as a separate Drive Doc and **attached to the content task** (`attachExistingDoc`), which becomes current and supersedes the displaced file. The task-attach (or a future task-close reconcile) is what seals the version; in-place editing + Google Docs revision history covers within-version edits. What shipped:
...
2. Adopt `<slug> yy-mm-dd-hh-mm` naming **on every file — active current included** (Plan B: timestamp everywhere, so a name-sorted library view is never a mix of bare + stamped names). Files are timestamped at birth: `createBlankDoc` (`LibraryService.js:463`) and the session fork path name new files `<slug> <ts>`; `attachExistingDoc` keeps the timestamp instead of force-renaming to the bare slug (`:538-539`). Timestamp = version birth (fork/create), not last-edit — in-place edits don't bump it (Google Docs revision history covers within-version). Resolution still rides the `slb_DocUrl` pointer; the `<slug> + space` newest-wins scan is the duplicate-disambiguation backstop. **Integrity matcher** (`HousekeepingService.runLibraryIntegrityReport`, `:1408`): change `slugSet.has(baseName)` to strip a trailing ` yy-mm-dd-hh-mm` and match the slug-prefix, since active files now carry the suffix too. **One-time migration:** a rename routine stamps all existing bare-`<slug>` assets to `<slug> <ts>` (ts from each file's Drive `getLastUpdated()`), so legacy actives match the new convention — not lazy-on-archive, which would leave a never-edited current bare forever.
3. Add the "Superseded by →" stamp + move to a single flat **`_archive`** subfolder under the existing `system.folder.library` (auto-minted on demand via the library service's existing `_getOrCreateChildFolder` helper — `LibraryService.js:391`), driven off the repoint breadcrumb. **No new top-level folder or SysConfig key** — `system.folder.library` already exists; the `_archive` underscore prefix avoids colliding with content `<type>` folders. **Do not use `system.folder.archive` / `JLMops_Archive`** — its 365-day trash sweep would delete history (see "Not `JLMops_Archive`" above). Also: skip the `_archive` subfolder in `HousekeepingService.runLibraryIntegrityReport`'s orphan-files walk (`HousekeepingService.js:1341`) so archived copies don't clutter the report (the slug-prefix matcher from step 2 already prevents them flagging as orphans; skipping the folder keeps the report scoped to live entities).
