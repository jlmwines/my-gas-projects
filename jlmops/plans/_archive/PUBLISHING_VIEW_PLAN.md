...
**Calendar access by sessions.** JLMops_Publishing is the shared publishing calendar. Sessions read it directly via Drive MCP (single-tab, compatible). No separate .md export.
...
| `JLMops_Publishing` | Shared publishing calendar (holidays + notes). Sessions read via Drive MCP; jlmops reads via SpreadsheetApp. |
...
- `jlmops/plans/OPS_SESSION_BRIDGE_PLAN.md` — jlmops-status.md export pattern (health + KPI)
