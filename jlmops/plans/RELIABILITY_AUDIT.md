...
**Status (2026-06-03). PART 1 SHIPPED (guard); rewrites DEFERRED.** Implementation step 3 (the empty/null guard) is live: `HousekeepingService` Phase 2 calls `reportFailure('tests.empty_or_null_result', High, ...)` when `TestRunner.runAllTests()` returns null or `total === 0`. Steps 1-2 (rewrite `ComaxAdapterTest` + `WebAdapterTest` to real invocation) and 4 (move `OrderServiceTest`/`ProductServiceTest` out) are DEFERRED to a from-editor session, because:
- **Risk is LOWER than this plan assumed** (good): both `ComaxAdapter.processProductCsv` and `WebAdapter.process*Csv` are *pure transforms* (input CSV blob/string → returned objects), writing to NO production tab. Comax's only side effect is a self-trashing temp Drive file. So the "tests must use dedicated `Test*` sheets / rollback via Drive history" caution below is **over-stated** — no `Test*` sheets are needed; in-memory fixtures suffice.
- **BUT validation is from-editor only.** A real happy-path fixture couples to the live column-map (`map.comax.product_columns` = 18 fields idx 0-17; `map.web.product_columns` headers `ID,SKU,Name,Published,Stock,Regular price`) **and** the `CmxProdS` schema (Comax validates the built product against schema headers + critical fields), and the Comax path runs a live Drive CSV→Sheet conversion. None of that is deploy-verifiable; the only green/red signal is running `TestRunner.runAllTests()` in the Apps Script editor. A session can't self-validate the rewrite, so it should be done when the user is at the editor to run the smoke. Do NOT ship rewritten suites blind.
