...
### Known issue — welcome-outreach recency + dedup gap (fix built, deployed, and verified live @568, 2026-08-27)

**Live verification, first run post-deploy (17:44-17:46 IL):** migration seed fired correctly on the real production sweep — `"Welcomed-emails list initialized with 11 existing recipient(s). No tasks created this run (migration seed)."` — zero errors, `createPendingPaymentFollowups` ran clean alongside it. Confirms the fix's most fragile step (first-run migration, avoiding a re-welcome flood) worked exactly as designed against live data.

**Problem**, confirmed by reading `HousekeepingService.js#createWelcomeOutreachTasks` (`:991-1093`): the sweep's only date check is a one-time floor (`system.crm.welcome_floor_date`), a lower bound stamped once at rollout — any first-completed order on/after that floor is equally eligible no matter how long ago it happened, contradicting "Fires once per customer" above, which implicitly assumed a *fresh* first order. Compounding it: `TaskService.createTask`'s dedup (`TaskService.js:170-178`) only blocks while a matching task's `st_Status` isn't `Done`/`Closed` — once a welcome task is closed, the same email is eligible again on the very next sweep, since nothing else remembers "already welcomed." Filed `.claude/bugs.md` 2026-08-21; user confirmed 2026-08-27 while investigating an unrelated old order surfacing a fresh welcome task.

**Fix 1 — recency window (upper bound).** Add a 7-day rolling window: eligibility becomes `fcd >= floorDate && fcd >= sevenDaysAgo` (in practice the 7-day bound dominates once the floor is more than a week old — both checks kept for clarity, not redundant removal). 7 days is a user call (2026-08-27): tight enough that "welcome" still reads as timely relative to the order, with slack for the sweep's own daily/20-min cadence. `sevenDaysAgo` computed fresh each run, not stored — no new config key for this half.

**Fix 2 — dedup that survives a closed task.** Mirror `crm.pending_payment_followup`'s own precedent (`HousekeepingService.js:1179-1189`, an explicit sent-tracking list) rather than changing `TaskService.createTask`'s dedup semantics globally — that function is shared by every task type in the codebase, and a status-blind dedup change there is a much larger, unreviewed blast radius than this fix needs. Add `system.crm.welcomed_emails` (JSON array of emails, runtime-written via `setConfigLocked`; register in `generate-config.js`'s `RUNTIME_KEYS`, default `'[]'`). Before calling `createTask` for an eligible email, check membership in this list and skip entirely if present — independent of whether a prior task is open, closed, or deleted. On successful creation, append the email and persist. `TaskService.createTask`'s own open-task dedup stays in place as a harmless secondary guard.

**Migration — avoid a backfill flood on first deploy.** The first run after this ships must not treat every already-welcomed customer (Done/Closed `task.contact.outreach` row today) as newly eligible. When `system.crm.welcomed_emails` doesn't exist yet: seed it from every email with any existing `task.contact.outreach` task (any status), persist, and create zero tasks that round — same shape as the existing floor-date first-run guard (`:997-1005`).

**Files touched:** `HousekeepingService.js#createWelcomeOutreachTasks` (both fixes + migration seed), `jlmops/generate-config.js` (new `RUNTIME_KEYS` entry). **Not building:** no change to `TaskService.createTask` itself, no change to the floor-date lower-bound mechanism (still correct as the "don't backfill pre-feature customers" guard), no UI change.

**Related efficiency fix, same session (built, deployed live @569):** `performFrequentMaintenance` ran both order-driven subtasks (`createWelcomeOutreachTasks`, `createPendingPaymentFollowups`) unconditionally every ~20-min cycle, each a full `WebOrdM` sheet read + scan, regardless of whether anything actually changed. `WooOrderPullService.pullOrders` already returns `orderCount` from a `modified_after` query (not `created_after` — confirmed by reading `WooApiService.js:302` — so a pure status transition on an existing order, e.g. Processing → Completed, bumps WooCommerce's `date_modified` and is included) and `OrderService.js#processStagedOrders` genuinely upserts pulled orders into `WebOrdM`'s status column by order ID, not insert-only (confirmed `:786-798`). So `orderCount === 0` on a successful pull is a safe, positive signal that `WebOrdM`'s completed/pending set is provably unchanged this cycle. Now skips both order-driven subtasks in that case (logging one line), and runs them normally on any pull failure (`orderCount` stays unresolved) rather than risk a silent miss. `checkCategoryStockHealth` stays unconditional — it's inventory-driven, unrelated to order changes.
