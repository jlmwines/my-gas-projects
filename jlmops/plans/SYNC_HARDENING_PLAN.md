...
**Created:** 2026-05-05
**Status:** **Bug 4 obsoleted 2026-05-12** — the `GENERATING_WEB_EXPORT` stage no longer exists. Web inventory export is now synchronous (`generateWebExportBackend` calls `ProductService.exportWebInventory` inline, transitions state directly). See `WEB_EXPORT_INLINE_PLAN.md` for the refactor. Bugs 1, 2, 3 still pending staging repro — backend looks clean. **2026-08-25: shipped a related batch of hardening on the inventory-push path** (cross-cutting #2, #3, #5 below, plus a new auto-retry item not originally in scope) — see the 2026-08-25 log entry.
**Scope:** Daily Sync widget UI/state drift. Excludes failed-Comax-import recovery (rare; tracked separately in `.claude/bugs.md`).

## Implementation log

- **2026-08-25** — Live incident on the `export.web.inventory.api` push (box 5 showed FAILED/red-X while the progress log showed a later success — a real instance of the "UI isn't a faithful mirror of backend state" problem this plan targets). Traced and shipped, jlmops @549:
  - **Cross-cutting #5 (button-enable invariant), scoped to Retry/Reset:** `retryFailed()` and `resetSync()` in `AdminDailySyncWidget_v2.html` never had the `actionInProgress` + button-disable guard that every numbered step button gets via `runAction()` — a double-click could fire the RPC twice. Both now use the same guard.
  - **New (not in original scope): job-level auto-retry.** `WooInventoryPushService._runPush` now retries just the still-failing rows up to 2 more times (3s apart) before marking the job FAILED, instead of failing the whole batch on the first transient error. Prompted by frequent server-side (not payload) failures — confirmed live same-payload retries succeed.
  - **Cross-cutting #3 (render gate), different mechanism than originally proposed:** two concrete bugs fixed instead of the `lastAppliedTimestamp` approach sketched below — `runAction()`'s success handler was logging a "done" checkmark based on `webExportFilename` presence rather than the actual returned stage/step status (a business-level failure could still render a misleading checkmark), and `refreshStatus()`'s polling loop had a `if (lastError) return` gate that permanently stopped background polling after any thrown RPC error, freezing the display until a manual refresh. Both removed/fixed; `lastError` variable removed as dead code.
  - **Error-message clobbering (new finding, same class as the 2026-08-21 Comax-import fix):** `_checkAndAdvanceSyncState`'s `PUSHING_WEB_INVENTORY` FAILED branch (`OrchestratorService.js`) wrote a generic `Status: FAILED` instead of the job's own per-SKU detail already sitting in `SysJobQueue.error_message`. Added `_getJobInfoBatch`/`getJobErrorMessageInSession` to surface the real reason.
  - **Log volume, unrelated but found during the same investigation:** `SyncStateService.setSyncState()` logged a row on every save regardless of whether the stage changed — now only logs on an actual stage change.
  - Smoke-tested same day on a real sync (no inventory changes to push) — ran clean end-to-end, confirming the changes didn't break the normal/no-op path. The retry/error-surfacing logic itself still hasn't been exercised by an actual transient failure — that remains the natural verification whenever one next occurs.
