...
**Created:** 2026-05-25
**Status:** SHIPPED — all phase 5 steps complete. Steps 1, 2, 3, 6 shipped 2026-05-25 (LibraryView scaffold + Outreach / Confirmation / deep-link packs). Step 4 (Content edit pack) + step 5 (Content publish pack) shipped 2026-05-27 with CONTENT_LIBRARY_PLAN phase 7b (`createBlankDoc` / `attachExistingDoc` / `lockVersion` / `logEntityActivity`). Phase 7a (LibraryService.addEntity + spawnContentChain + Create Content Tasks admin button + modal) shipped 2026-05-26. **Phase 9 entity detail drawer shipped 2026-05-27** end-to-end (9a scaffold + 9a polish + 9b sections + 9c back-stack nav + 9d-partial 3 action buttons; Lock intentionally NOT a drawer action — task-pack only). LibraryView surface is feature-complete for the planned manager + admin workflows. **Phase 11 distribution events fully shipped 2026-05-28**: email v1, news v1, social v1 all live. **`library.enabled` flag RETIRED 2026-05-28** — flag deleted from SysConfig source, all `if (libraryEnabled)` short-circuits removed from code, AppView nav entries unconditional, `_libraryEnabledOrError` helper deleted. Library subsystem is now permanent. Below references to `library.enabled` describe historical gating; treat as past tense. Future work continues in CONTENT_LIBRARY_PLAN.md §17 phases 12 / 13 / 14 / 15.
