...
**Holiday and non-campaign markers** appear as non-clickable date rows (distinct style). Source: `JLMops_Calendar` workbook — a flat Google Sheet maintained by sessions via Drive MCP. Columns: date · name · type (holiday / blackout / note) · notes. Jlmops reads this sheet at Calendar load time.
...
| `JLMops_Calendar` | New standalone Google Sheet (flat, Drive-MCP-writable). Holiday/blackout source for Calendar tab. |
