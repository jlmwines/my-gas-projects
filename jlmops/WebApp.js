...
const VERSION = {
  built: '2026-05-13 02:28',
  commit: '@94: three tech-debt fixes in one deploy. (1) Comax order export — bundle parent SKUs are now filtered out of OrderService.exportOrdersToComax. The aggregation loop builds a bundleSkus Set from WebProdM (wpm_TaxProductType in {woosb, bundle}) and skips parent rows; member lines export as before. Same pattern that already lives in InventoryManagementService.generateComaxInventoryExport. Fixes a regression where bundle parents were appearing in the Comax CSV alongside their member SKUs. (2) Sync state-machine — failed IMPORTING_COMAX now drops the user back to WAITING_COMAX_IMPORT on Retry (the upload UI), instead of back to IMPORTING_COMAX. Mirrors the existing precedent at the PUSHING_WEB_INVENTORY → WAITING_WEB_CONFIRM special case in retryFailedStepBackend. User can now drop in a corrected CSV after a parse failure without needing manual sheet intervention. (3) CRM — gift detection simplified per CRM_PLAN.md §255-263: gift iff customer note is non-empty AND shipping last name differs from billing last name. Dropped the DELIVERY_KEYWORDS exclusion. correctContactData() also aligned: war-support no longer triggers off-core (CRM_PLAN.md §279 — war-support category left alone, self-resolves via Lapsed/Dormant aging). Archive-mapping CouponItems was already present in code; stale "considers delivery keywords" comment in ContactImportService.js removed.'
};
