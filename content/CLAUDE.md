...
| `## EMAIL PREVIEW TEXT` | 150 chars max | Preheader shown in email clients before opening — most clients truncate past 150. Session drafts in English only; manager translates. |
| `## EMAIL BODY` | full email copy | Companion email body — post-promo-led, features this post (see `marketing/_resources/NEWSLETTER_REFERENCE.md` Companion Email Campaign). Not part of the body-drafting pass — drafted in the later derivative pass, only after the manager has returned the body locked; session drafts in English only, manager translates. |
| `## EMAIL CTA` | one line | Email call-to-action button/link text. Session drafts in English only; manager translates. |
...
9. **CTA**
10. **Image prompts**
11. **HE translation — the manager's step, not the session's.** Once the full EN package (steps 4–10) is complete, correct, and verified against the locked body (character limits, no staleness), hand it to the manager as one package. He translates everything himself — Title, body, Excerpt, Email fields, Newsletter Excerpt, Print Newsletter Body, CTA. The session never originates Hebrew for any of it, not even as a flagged "draft for review." A missing Hebrew field means the manager hasn't translated it yet, not a gap for the session to fill. (Real incident 2026-08-17, Galilee post: session drafted placeholder Hebrew Title/CTA to patch what turned out to be a stale/oversized EN meta description the manager couldn't have translated yet — the fix was correcting the EN source, not guessing at Hebrew. See `feedback_blog_post_pipeline_manager_translates_full_set`.)
