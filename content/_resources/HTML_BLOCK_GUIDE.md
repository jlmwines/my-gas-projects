...
Before writing a single line of column/image HTML: look at `content/_resources/existing-layouts/*.raw.txt` (raw HTML dumps of already-published posts) and an already-shipped bilingual pair like `content/basics/handling/Handling EN.post.md` / `Handling HE.post.md`. Copy the actual structure — don't invent column widths, classes, or CSS from first principles. This mirrors the jlmops UI rule ("find the same element type already working and copy it exactly") — it applies just as much to content HTML.
