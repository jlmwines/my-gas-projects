...
1. **WPML String Translation cleanup** — first PO import done 2026-05-06: 35 customer-facing WC strings populated with canonical HE from translate.wordpress.org via `jlmwines/exchange/strings/woocommerce-he-customer-facing.po` (WPML → String Translation → Import, "Also create translations" checked, domain `woocommerce`). **Verification deferred** — user to walk HE storefront after cache flush (WPML → Support → Reset cache + SG Optimizer purge), spot-check imported strings render in HE (e.g., footer "תנאי שימוש"/"מדיניות פרטיות", checkout coupon entry, 404 page "העמוד לא נמצא"). Then: retire the `gettext`/`ngettext_with_context` filters in `inc/shop-filters.php` for the catalog result-count phrases and WC dropdown labels — these now duplicate the imported translations and should be removed once WPML rows are confirmed authoritative. The remaining ~290 WPML-untranslated WC strings (admin/blocks/dev) deferred — they don't render to customers; not worth the maintenance cost. Tooling: `jlmwines/exchange/strings/lookup-canonical.js` reusable for any future canonical-translation pulls.
...
8. **Homepage architecture rebuild — queued, not for today.** Current setup (Settings → Reading = "Your latest posts", `front-page.php` overrides for the front-page URL, no Page records anchoring the homepage) is the root cause of several SEO friction points: hreflang `http://` on homepage only, `/he/home-elegant/` stray sitemap entry, no per-page RankMath fields working on the homepage. A prior session converted to this template-only structure without surfacing the trade-off; flagged as a behavioral correction in memory. Plan written at `website/HOMEPAGE_BLOCKS_PLAN.md` — two phases: **Phase 1 interim** (~1–2 hrs): rename `front-page.php` → custom page template, assign to the existing EN + HE `home-elegant` Pages, switch Settings → Reading to "A static page" → fixes all SEO surface issues without rebuilding the carousels. **Phase 2 full** (~2–3 sessions): build `jlm/product-carousel` + `jlm/post-carousel` Gutenberg blocks, rewrite homepage Page content using the blocks, retire the custom template — fixes editability. Phase 1 unblocks SEO immediately; Phase 2 is the architecture cleanup that addresses the carousel-reusability story (carousels already used 2+ times on homepage with different settings).
9. **Unused plugin cleanup on live** — delete deactivated plugins to reduce attack surface and update noise. **DO NOT DELETE `mailchimp-woocommerce`** — its `wp_options` API key is what the theme's replacement code reads. Safe to delete: `woo-smart-wishlist`, `WPBingo`, `redux-framework`, `Elementor` + `Elementor Pro` (after verifying no remaining Elementor pages — search wp-admin → Pages for the Elementor edit indicator), `widget-importer-exporter`, `better-search-replace`, `wp-file-manager`. Verify-before-delete: `contact-form-7` + CF7 multilingual (any forms in use?), `variation-swatches-for-woocommerce` (variation selector acceptable without it?), `woocommerce-checkout-field-editor` (any custom checkout fields configured?).
