...
**Updated:** 2026-08-05. jlmops @544 live, stable, unchanged since the 2026-07-26 WebXltM incident fix (see `.claude/bugs.md` for that history). Marketing-side: Meta Ads testing method changed to Meta's native Dynamic Creative (multiple variants in one ad, algorithm-optimized) rather than manual sequential rounds. Reel 1 (bucket 8, "Calendar of confidence") built and edited; Reel 2 (bucket 14, real Google reviews over a real photo of Evyatar) user-reported complete 2026-08-05. Neither yet uploaded to Meta Ads Manager. Flyer Round 1 (Talbiye) redemptions still too soon to read; Round 2 (Emek Refaim) targeted 3rd week of August.
...
- **marketing** — flyer active (Round 1 pending, Round 2 planned); Meta Ads on Dynamic Creative method, 2 reels built (bucket 8 calendar, bucket 14 reviews), not launched; newsletter cadence current.
...
4. **Meta Ads Jerusalem test — method changed 2026-08-04 to Dynamic Creative (Meta's native multi-variant-in-one-ad optimization), replacing the earlier manual sequential-round plan.** `marketing/plans/META_ADS_PLAN.md` has the full candidate library (14 buckets + fresh-eyes pool) plus two as-built reels: bucket 8 ("Calendar of confidence") and bucket 14 (real Google reviews, 4.9★/148 reviews, over a real photo of Evyatar — user-reported complete 2026-08-05, not yet verified by this project). Next production candidate: bucket 7's revised split-screen concept (makolet shelf vs. Evyatar's shelf, green checks), then buckets 9 and 3. Still open before launch: set up the Dynamic Creative ad in Ads Manager (one ad set, multiple image/text variants), source background music for both finished reels (direction already set in the plan), and check the two unverified shortcuts noted previously (Ads Manager's "Import ads in bulk" menu; a claimed official Meta MCP server) before defaulting to a manual campaign build.
