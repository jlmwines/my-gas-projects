...
**Updated:** 2026-07-30. jlmops @544 live, stable, unchanged since the 2026-07-26 WebXltM incident fix (see `.claude/bugs.md` for that history). This session's work was entirely marketing-side: a Meta Ads Jerusalem test plan (`marketing/plans/META_ADS_PLAN.md`) was built out extensively — recovered from an unmerged branch, developed through a wide candidate creative library (12 buckets + a 24-concept fresh-eyes pool), then one candidate (bucket 8, "Calendar of confidence," the user's stated favorite) moved from concept into active Canva production — real assets found and being assembled (delivery clip, calendar background, a finalized 7-icon set). Still not launched — no campaign built, no round run — but meaningfully closer than a pure concept. Flyer Round 1 (Talbiye) redemptions still too soon to read; Round 2 (Emek Refaim) targeted 3rd week of August.
...
4. **Meta Ads Jerusalem test — bucket 8 mostly down to Canva execution mechanics now; still not launched.** `marketing/plans/META_ADS_PLAN.md` has the full candidate library (12 buckets + fresh-eyes pool) plus a live-tested shot list for bucket 8 ("Calendar of confidence," the user's favorite, not formally locked). Tested and working: background/phone/calendar-grid layers, a consistent six-icon set, the resolution box icon, and the real site homepage as Evyatar's asset (reused for both the small resolution-beat inset and the full-bleed breakout reveal). Still open, in rough build order: doorbell/box opening (fisheye crop + logo tease not built), six text labels (decided, not yet tested as an overlay), full icon+label timing on one page, the breakout zoom+fade, the five-line closing captions (copy never drafted — a content gap, not just a build step), the offer end-card copy, and the corner box glow/pulse (needs a live motion test). Session paused for a break 2026-07-31; punch list is the pickup point.
...
7. **Galilee region post (Slot B, due 2026-08-11, see `content/plans/REGION_POSTS_PLAN.md`)** — in progress; drafted + registered in the library (`blog-region-galilee-en`, state `draft`). HE translation also already registered (`blog-region-galilee-he`, state `draft`) — a Library-tab display bug (fixed this session) had been hiding it from view.
8. **Central Mountains region post (Slot C) — body drafted through Image Prompts, 2026-07-09.** Calendar row staged (`blog-region-central-mountains`, cal_Date 2026-08-25, pending "Apply Pending Updates"); Drive doc placed at the canonical library path (`blog-region-central-mountains-en 26-07-09-12-05`); git source at `content/regions/central-mountains/central-mountains-en.post.md`. Remaining: winery verification (Gvaot/Tura not yet confirmed against JLM's carried wineries), Canva images, HE translation, library registration, WP push — same checklist as Galilee (`content/plans/REGION_POSTS_PLAN.md`).
