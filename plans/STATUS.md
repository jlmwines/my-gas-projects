...
**Updated:** 2026-07-06 — @442 live; Admin Bundles message-strip close bug fixed; Library Doc-ownership gap found and partially fixed (`createTranslationDraft` transfers HE-copy ownership to admin going forward; generic `attachExistingDoc` path still open). **Negev region post is live, both languages** (https://jlmwines.com/negev-wine/, https://jlmwines.com/he/blog-negev/ — note EN/HE slugs no longer match) — wp-admin finishing (WPML link, SEO meta, focus keyword) still pending. New `content/HTML_BLOCK_GUIDE.md` documents the post-HTML column/image convention (HE mirrors EN's structure exactly, no cross-language flipping) after a real rabbit hole building Negev's layout from scratch instead of copying precedent. Wine Talk category groundwork shipped live (Wine Basics renamed, Regions category created). Galilee drafted + registered; Grapes anchor drafted, facts verified.
...
6. **Negev region post — published live in both languages, 2026-07-06.** EN: https://jlmwines.com/negev-wine/ (post 67600). HE: https://jlmwines.com/he/blog-negev/ (post 67602). Slugs no longer match between languages (EN changed from the original `blog-negev` draft slug to `negev-wine` during wp-admin finishing). Layout iterated heavily this session (paragraph spacing, column splits, mobile image order, winery grouping+links+map plan) — see `content/HTML_BLOCK_GUIDE.md` for the resulting convention doc. **Remaining wp-admin finishing:** (a) HE SEO meta description (not yet drafted); (b) focus keyword both languages; (c) WPML-link the two posts as translations of each other (`push-posts.js` doesn't do this); (d) winery verification (confirm which Wineries to Visit entries JLM actually carries, add shop links) — not yet done, should happen even though already published.
7. **Galilee region post (now Slot B, due 2026-08-11 — reshuffled after Negev published out of sequence, see `content/REGION_POSTS_PLAN.md`)** — drafted + registered in the library (`blog-region-galilee-en`, state `draft`), ready for the manager. Deliberately not yet flagged/pushed to the manager — hold until Negev clears the pipeline above.
...
- **Content Library** — entity/task/activity-log model live; Doc-sourced EN/HE templates, attach-to-replace versioning (now reachable directly from the Library detail drawer, not just via a task). `LibraryView.html` was promoted to `PublishingView.html` per `jlmops/plans/_archive/PUBLISHING_VIEW_PLAN.md` — **confirmed live 2026-07-07**, not catalog-only as previously stated here: admin sees Calendar/Library/Campaigns/Projects tabs, manager sees Calendar/Tasks/Library. Calendar tab surfaces every Library entity with `slb_TargetDate` set (task lens itself still lives in AdminTasksView + manager dashboard, unchanged). **Known gap found 2026-07-07:** no function exists to edit or clear `slb_TargetDate` on an existing entity after creation — target date is create-only (`register-library.js`'s `target_date`, or `spawnContentChain`'s `targetDate` param). This is why the Calendar/Deficiency view accumulates overdue items with no way to reschedule or dismiss them; needs a scoped fix (likely an admin-only update action alongside the existing `WebAppPublishing_setEntityCampaign` precedent). Migration scaffolding to retire: `createBlankDoc` seeding, inline `slb_Body`, Create-Doc button. Phase 12 cross-link renderer blocked on regions overhaul. System doc → `jlmops/docs/DATA_MODEL.md` (Content Library Data Model); design history archived at `jlmops/plans/_archive/CONTENT_WORKFLOW_REDESIGN_PLAN.md` + `PUBLISHING_VIEW_PLAN.md`.
