...
**Updated:** 2026-06-07 — jlmops @233 · theme v1.2.29 live. BUNDLE_PLAN **Stages 0–2 COMPLETE** (@227→@233): Stage 0 qty=0 price fix, Stage 1 Bundles-view perf, Stage 2 cost/profit data layer (`cpm_Cost` on CmxProdM + `wpm_ProfitRate` on WebProdM, ex-VAT 18% margin, preserve-only rate, AdminInventory "Product Costs & Margins" card — user-verified live). **Standing jlmops backlog:** BUNDLE_PLAN Stages 3–7 (`jlmops/plans/BUNDLE_PLAN.md` — Stage 3 authoring + woosb export is the next core win); reliability-audit (`RELIABILITY_AUDIT.md`, partial) + UI-audit (`UI_AUDIT.md`) queues; `TEST_HARNESS_PLAN.md` (pending); CODE_AUDIT Phase 1 authz (`CODE_AUDIT_PLAN.md`); RELOAD_RESILIENCE Option A (deferred behind A0). (Narrative → `.claude/session-log.md`.)
...
| Revenue | Steady |
| Deploy Version | jlmops deploy @233 · theme v1.2.29 LIVE |
| Deploy Date | jlmops 2026-06-07 (deploy @233) · theme 2026-06-04 (v1.2.29) |
...
- **Bundle handling — `jlmops/plans/BUNDLE_PLAN.md`** (consolidated staged master plan). **Stages 0–2 COMPLETE @227→@233** (re-sequenced numbering): **0** qty=0 price fix; **1** Bundles-view perf (N+1 + mount); **2** cost/profit data layer (`cpm_Cost`/`wpm_ProfitRate`, ex-VAT margin, preserve-only rate, AdminInventory margins card — user-verified). Remaining: **3** authoring + woosb export (the core win); **4** EN/HE parity + stock (mostly already built); **5** fast bundles-only member refresh; **6** cross-bundle diversity; **7** unified suggestion generator. REST push parked (vendor import safer). Implement depth decided per-stage.
