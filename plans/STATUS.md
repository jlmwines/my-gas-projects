...
**Updated:** 2026-07-22. jlmops @519 live, stable; `SysCategories` consolidated as the category authority (see `jlmops/plans/CONFIG_COMPLIANCE_PLAN.md`, one cleanup step held for a production soak period). Woo API push plan under active feasibility review — a diagnostic-only test function is live, awaiting the owner running it before the plan's attribute-ID approach is finalized.
...
| Revenue | Steady |
| Deploy Version | jlmops @519 · theme v1.2.31 |
...
- **Woo API push (descriptions, category, attributes)** — fully scoped plan at `jlmops/plans/WOO_API_PUSH_PLAN.md`. Its generator-duplication fix already shipped standalone, and category-ID sourcing is done (`SysCategories.sct_Value`, via `CONFIG_COMPLIANCE_PLAN.md`). Remaining scope (attribute-ID sourcing, export rework, push service) is under active feasibility review — the export will stay Sheet-based rather than convert to CSV, and the push will extend the existing live `WooInventoryPushService.js` rather than build new; a live diagnostic test is deployed and awaiting the owner's run to settle attribute-ID sourcing before the last piece is finalized. New product records on hand in the meantime go through the existing manual export — the remaining build isn't ready to trust for a live push yet. Brand dropped from scope entirely: no brand data exists today, and Google Shopping already maps Winery→Brand with high visibility, so a native Brand field closes no real gap. Winery custom attribute stays in the push as a future hook. Sequencing open: this is catalog/ops infrastructure, not one of the acquisition-period's named channels. Once built, extends to carry upsell/cross-sell values (separate later plan).
