...
**Updated:** 2026-05-18 (mobile LCP rollback shipped as theme v1.2.26+v1.2.27 — reverted hero preload + logo fetchpriority strip; KEPT hero `<picture>`+WebP, block-CSS dequeues, Elementor dns-prefetch strip; LCP back to baseline 4.1, FCP 3.6 stable across SG-cache-flushed re-runs — 0.4s vs baseline 3.2 unattributable to any reverted change, likely PSI single-measurement variance; magnums removal `4b630c9` also shipped this session as side-effect of incremental FTP push; next: render-blocking pile (main.css 26.6 KiB / 1,080 ms + jquery 35.1 KiB / 1,390 ms) is the actual LCP lever — separate plan, non-trivial regression risk)
...
9. **Remove "Magnums" product category** — magnums never were in bundle compositions (earlier wording was wrong); just a section on the gifts page + a category term. **Template fix SHIPPED 2026-05-18 (`template-gifts.php` via `4b630c9` + side-effect of v1.2.26 FTP push)** — section removed from gifts template, hero anchor chip auto-disappears. **Still pending:** user-side WP admin step to delete the EN+HE `product_cat` term pair.

**Other in-flight initiatives** (cross-area, strategic context in `business/STRATEGY.md`):
- **Content Library — PLAN WRITTEN 2026-05-17 `plans/CONTENT_LIBRARY_PLAN.md`.** Major architectural plan after Frankenstein-pattern reflection. Library = entity layer for content/marketing/CRM work; three primitives (entities, tasks, activity logs); references between entities (no parent/child); templates migrate from SysConfig over time; Campaigns absorbed; CRM stays as-is. Prerequisites: lookup-add UI (bug filed), library schema design, Claude-readable single-tab Drive export pattern. Implementation pending — not started; user wanted overnight to sit with the architecture before committing.
