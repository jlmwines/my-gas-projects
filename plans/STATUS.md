...
**Updated:** 2026-06-04 — jlmops @226 · theme v1.2.29 live. **Today:** theme SKU search restored in the WPClever bundle builder (v1.2.29); Sync-view literal-scriptlet fix (@223); **reload-resilience A0 SHIPPED + user-verified** — count entry autosaves to `sessionStorage` and survives mobile pull-to-refresh (@224, `jlmops/plans/RELOAD_RESILIENCE_PLAN.md`); count-entry modal overlay fix (@225); mobile horizontal padding minimized app-wide (@226). **New plan docs:** `jlmops/plans/BUNDLE_AUTHORING_EXPORT_PLAN.md` (author bundles in jlmops → export woosb meta to WC; ready to build) and `jlmops/plans/CODE_AUDIT_PLAN.md` (reviewed; authorization gap real — only 2/15 `WebApp*` controllers gate on `AuthService`). **Standing jlmops backlog:** reliability-audit (`RELIABILITY_AUDIT.md`, partial) + UI-audit (`UI_AUDIT.md`) queues; `TEST_HARNESS_PLAN.md` (pending); RELOAD_RESILIENCE Option A / server-draft (deferred behind A0). (Narrative → `.claude/session-log.md`.)
...
- **Bundle management** — composition / member-condition split is SHIPPED (refreshBundleComposition + checkBundleHealth as separate housekeeping steps, post-sync auto-trigger @82, sync widget Run Bundle Analysis button @95). Next: `jlmops/plans/BUNDLE_MANAGEMENT_REFINEMENTS_PLAN.md` (written 2026-05-13) — bundle card UI cleanup (3 named buttons replacing Refresh/Add-New/Re-import) + new EN/HE composition parity validator (atomic (product_id, qty) pair check, section-aware, EN-as-truth). Implementation pending OK. **NEW 2026-06-04: `jlmops/plans/BUNDLE_AUTHORING_EXPORT_PLAN.md`** (ready to build) — author composition in the existing jlmops editor, then export the two `woosb_ids` blobs (EN/HE) as a table of only edited bundles and paste into WPClever's first-party import. jlmops = authoring surface, WC = system of record (`sb_PendingExport` is volatile, the daily refresh re-derives + clears it). This supersedes the REST-push idea: `BUNDLE_API_PUSH_TEST_PLAN.md` is **parked** (export via the vendor's own import is safer than a raw REST write). Longer-term considerations fenced in the plan: Comax-cost margin-aware inclusion + cross-bundle diversity.
