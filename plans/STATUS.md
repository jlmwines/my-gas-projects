...
- **Bundle handling — `jlmops/plans/BUNDLE_PLAN.md`** (consolidated master plan, 2026-06-04; supersedes `BUNDLE_AUTHORING_EXPORT_PLAN` + `BUNDLE_MANAGEMENT_REFINEMENTS_PLAN`, both now stubs). One staged plan, implement depth TBD as it's refined: **0** qty=0 price-bug fix; **1** bundles-only direct-to-master pull (fast member refresh) + view load control; **2** author composition in the jlmops editor → export `woosb_ids` table (EN/HE, only edited bundles) → paste into WPClever's first-party import (jlmops = authoring surface, WC = system of record, `sb_PendingExport` volatile); **3** EN/HE parity validator + stock review; **4** Comax-cost whole-bundle margin (discounted price − Σ cost×qty); **5** cross-bundle diversity/rotation. `BUNDLE_API_PUSH_TEST_PLAN.md` parked (vendor import safer than REST push).
