...
**Updated:** 2026-05-19 (Context post LIVE EN+HE — https://jlmwines.com/context/ + /he/context/ — with featured image 67422, body images 67413/67414 (v2 swaps), 4 cross-links to pairing/intensity/acidity/complexity posts, mobile cols-flip pattern keeping image-then-text consistent across alternating layout; MCP capability audit completed — Mailchimp/Canva/Drive scopes documented as feedback memories, three untested MCPs (Gmail/Calendar/WordPress.com) added to Inbox for vetting before any future plan names them; `marketing/NEWSLETTER_PLAN.md` major revision — pandoc + reference docx templates do the layout work, Canva role narrowed to image-gen only, Drive MCP delivers finished docx; new "Making Wine — [topic]" series concept introduced as recurring subscriber-exclusive monthly journal of vineyard/winery activity, lives only in print newsletter + companion email, not on blog)
...
- **Content Library — PLAN WRITTEN 2026-05-17, REFINED 2026-05-18 `plans/CONTENT_LIBRARY_PLAN.md` (committed `f59d23e`).** Major architectural plan after Frankenstein-pattern reflection. Library = entity layer for content/marketing/CRM work; three primitives (entities, tasks, activity logs); references between entities (no parent/child); templates migrate from SysConfig over time; Campaigns absorbed; CRM stays as-is. 2026-05-18 refinement added: slug conventions (§20 NEW — `<type>-<topic-or-series>-<discriminator>[-<language>]` with hyphens, type-first); short type vocabulary (blog/news/mention/email/social/image/template/customer); sibling-language as default rule; template channel-language split (4 entities per use-case); MD-local + DOC-in-Drive (revises original "Drive is truth"); KPI ingest pattern (§21 NEW). Prerequisites: lookup-add UI (bug filed) is §17 phase #1 blocker before anything else. Implementation pending.
