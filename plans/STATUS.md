...
**Updated:** 2026-08-10. jlmops @544 live, stable, unchanged since the 2026-07-26 WebXltM incident fix (see `.claude/bugs.md` for that history). Marketing-side: Meta Ads Jerusalem round 1 (relaunched 2026-08-07) still running, results pending. Round 2 candidate library expanded to 20 creative buckets, with two pieces fully specced and build-ready (Occasion Rack, "Outsource It" — real, verified wine-authority quotes) — see `marketing/plans/META_ADS_PLAN.md`. Flyer Round 1 (Talbiye) redemptions still too soon to read; Round 2 (Emek Refaim) targeted 3rd week of August.
...
- **marketing** — flyer active, Meta Ads round 1 live with results pending, round 2 creative pipeline build-ready; newsletter cadence current.
...
4. **Meta Ads Jerusalem test — round 1 (launched 2026-08-06) superseded 2026-08-07.** A Meta Ads Manager overlay warning on the original reels' captions triggered a caption/safe-zone fix (60pt captions, dark gradient band, content confirmed in safe zone) across all 3 reels (bucket 8 "Calendar of confidence," bucket 14 real Google reviews, bucket 7), plus differentiated landing pages per ad (bucket 8 + 7 → special-value bundle product page; bucket 14 → Bundles category page) and a relaunch — still 3 separate ads in one ad set, ₪550 through end of August, Sun–Thu, objective web traffic (Pixel reinstalled and confirmed firing, but kept as a secondary signal — volume too thin for Purchase/Add-to-Cart optimization). See `marketing/plans/META_ADS_PLAN.md` Round Log. Next: confirm the relaunch is live, confirm background music made it into the uploaded files, then let it run its minimum duration before reading results (Confirmation checklist). Round 2 candidate library expanded to 20 creative buckets with two pieces fully specced and build-ready (Occasion Rack, "Outsource It") — pick up production once round 1's results are in.
5. **Server-side authorization program** — the code audit's one unresolved finding: `doGet` is a login gate, not an authorization boundary; the role-switcher is client-side only and no mutating endpoint re-checks caller role. Needs a dedicated `AUTHORIZATION_PLAN.md` (sketch in `jlmops/plans/CODE_AUDIT_FIX_SEQUENCE.md` Session R) — a program, not a quick session.
6. **Ongoing operational cadence** (continuous): update products; validate web product data + image accuracy (`jlmops/plans/PRODUCT_VERIFICATION_PLAN.md`); add products to fill category gaps; publish regularly (blog pipeline + monthly newsletter).
7. **Galilee region post (Slot B, due 2026-08-11, see `content/plans/REGION_POSTS_PLAN.md`)** — in progress; drafted + registered in the library (`blog-region-galilee-en`, state `draft`). HE translation also already registered (`blog-region-galilee-he`, state `draft`) — a Library-tab display bug (fixed this session) had been hiding it from view. Companion email calendar row (`email-region-galilee`, cal_Date 2026-08-11) staged 2026-08-10 — was missing from `JLMops_Publishing`, pending "Apply Pending Updates."
8. **Central Mountains region post (Slot C) — body drafted through Image Prompts, 2026-07-09.** Calendar row staged (`blog-region-central-mountains`, cal_Date 2026-08-25, pending "Apply Pending Updates"); Drive doc placed at the canonical library path (`blog-region-central-mountains-en 26-07-09-12-05`); git source at `content/regions/central-mountains/central-mountains-en.post.md`. Remaining: winery verification (Gvaot/Tura not yet confirmed against JLM's carried wineries), Canva images, HE translation, library registration, WP push — same checklist as Galilee (`content/plans/REGION_POSTS_PLAN.md`). Companion email calendar row (`email-region-central-mountains`, cal_Date 2026-08-25) also staged 2026-08-10, pending merge.
9. **Grapes anchor post** ("Grape Varieties in Israel") — drafted through Image Prompts + Notes at `content/grapes/grapes-en.post.md`, facts verified. Not yet registered in the library or flagged to anyone. Individual grape spoke posts deferred per `content/plans/ISRAELI_WINE_GUIDE_PLAN.md`'s sequencing decision.
10. **Vintage-drift smoke test** — fix deployed @528 (`jlmops/plans/VINTAGE_MISMATCH_SNAPSHOT_FIX_PLAN.md`); needs a live vintage change to occur naturally before it can be confirmed working (no way to manufacture the test condition). Also: the task open since 2026-07-18 still carries the old, pre-fix stale snapshot — user to correct/clear it directly.
