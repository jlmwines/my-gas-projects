...
4. **Meta Ads Jerusalem test — bucket 8 reel built, edited, and live-tested at 23.9s; not yet launched.** `marketing/plans/META_ADS_PLAN.md` has the full candidate library (12 buckets + fresh-eyes pool) plus the as-built bucket 8 recipe ("Calendar of confidence," the user's favorite, not formally locked). Asset production is done — icons, calendar, logo-on-box (distorted + blurred, seamless loop confirmed), Evyatar's growth into the real homepage, bundles-page offer card, all tested live in Canva. Only genuinely open item: the corner-glow idea on the resolution box was never confirmed either way — treat as dropped unless revisited. Next: build the base campaign manually in Ads Manager (Prerequisites/Step 1) and check the real safe-zone overlay against the uploaded file — the box/logo positioning question can only be verified there.
