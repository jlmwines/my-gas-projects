...
**Updated:** 2026-08-03. jlmops @544 live, stable, unchanged since the 2026-07-26 WebXltM incident fix (see `.claude/bugs.md` for that history). Marketing-side: the Meta Ads Jerusalem test's bucket 8 concept ("Calendar of confidence") is now a fully built and edited 23.9s reel, live-tested in Canva across several sessions — not yet uploaded to Meta Ads Manager or launched. Flyer Round 1 (Talbiye) redemptions still too soon to read; Round 2 (Emek Refaim) targeted 3rd week of August.
...
- **marketing** — flyer active (Round 1 pending, Round 2 planned); Meta Ads bucket 8 reel built and edited (23.9s), not launched; newsletter cadence current.
...
4. **Meta Ads Jerusalem test — bucket 8 reel built, edited, and live-tested at 23.9s; not yet launched.** `marketing/plans/META_ADS_PLAN.md` has the full candidate library (12 buckets + fresh-eyes pool) plus the as-built bucket 8 recipe ("Calendar of confidence," the user's favorite, not formally locked). Asset production is done — icons, calendar, logo-on-box (distorted + blurred, seamless loop confirmed), Evyatar's growth into the real homepage, bundles-page offer card, all tested live in Canva. Only genuinely open item: the corner-glow idea on the resolution box was never confirmed either way — treat as dropped unless revisited. Next: check two unverified shortcuts before defaulting to a manual build — Ads Manager's separate "Import ads in bulk" menu (third-party sources claim blank-ID rows create new campaigns, unconfirmed) and a claimed official Meta MCP server (unconfirmed against primary sources, see `META_ADS_PLAN.md`). Otherwise build the base campaign manually (Prerequisites/Step 1) and check the real safe-zone overlay against the uploaded file — the box/logo positioning question can only be verified there.
