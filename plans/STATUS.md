...
**Updated:** 2026-07-30. jlmops @544 live, stable, unchanged since the 2026-07-26 WebXltM incident fix (see `.claude/bugs.md` for that history). This session's work was entirely marketing-side: a Meta Ads Jerusalem test plan (`marketing/plans/META_ADS_PLAN.md`) was built out extensively — recovered from an unmerged branch, then developed through a wide candidate creative library (12 buckets + a 24-concept fresh-eyes pool), Canva production mechanics, and the two-Claude-account workflow (this project plans/drafts, the manager executes in Meta Business Platform by hand; jlmops's only role is a task+library notification, nothing more). Nothing is decided yet — no campaign built, no round run; nailing down Round 1's actual pick and assets is the next step. Flyer Round 1 (Talbiye) redemptions still too soon to read; Round 2 (Emek Refaim) targeted 3rd week of August.
...
- **marketing** — flyer campaign active (Round 1 redemptions pending, Round 2 planned); Meta Ads test plan drafted (`marketing/plans/META_ADS_PLAN.md`), not launched; newsletter cadence current.
...
3. **Flyer advertising — Round 1 (Talbiye, 5,000 residences) distributed 2026-07-26/27; site traffic confirmed via campaign tag, redemptions pending (too soon to read).** Round 2 target selected: Emek Refaim, 3rd week of August (light-rail disruption there + summer-travel timing). Plan → `marketing/plans/FLYER_PLAN.md`. Coupon rides the offline-attribution scheme (Inbox, `defer:2026-08-10`).
4. **Meta Ads Jerusalem test — plan built out, nothing decided or launched yet.** `marketing/plans/META_ADS_PLAN.md` has a wide candidate creative library (12 buckets + fresh-eyes pool), production mechanics, and the operating workflow (this project plans/drafts, the manager executes in Meta by hand). Next real step: pick Round 1's actual candidate (bucket 8, "Calendar of confidence," is the user's stated favorite but not locked) and produce its assets.
