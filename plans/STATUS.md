...
**Updated:** 2026-06-07 — jlmops @244 · theme v1.2.29 live. BUNDLE_PLAN **Stages 0–2 COMPLETE + Stage 3 CORE shipped** (@227→@242): Stage 0 qty=0 price fix; Stage 1 Bundles-view perf; Stage 2 cost/profit data layer (`cpm_Cost`/`wpm_ProfitRate`, ex-VAT 18%, AdminInventory margins card); **Stage 3 core = serializer (`exportBundleWoosb`) + Export worklist on AdminBundles (ops≠web product-multiset diff) + HE text capture — verified live (flags only real product changes).** Remaining Stage 3: profit-in-selector, as-presented profit, out-of-stock failsafe. **Standing jlmops backlog:** BUNDLE_PLAN Stage 3 finish + Stages 4–7 (`jlmops/plans/BUNDLE_PLAN.md`); AdminBundles UI overhaul (`ADMIN_BUNDLES_UI_PLAN.md`, Dispatch-led, design defined); reliability-audit + UI-audit queues; `TEST_HARNESS_PLAN.md`; CODE_AUDIT Phase 1 authz; RELOAD_RESILIENCE Option A. (Narrative → `.claude/session-log.md`.)
...
| Revenue | Steady |
| Deploy Version | jlmops deploy @244 · theme v1.2.29 LIVE |
| Deploy Date | jlmops 2026-06-07 (deploy @244) · theme 2026-06-04 (v1.2.29) |
