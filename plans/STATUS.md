...
- **Woo API push (descriptions, category, attributes)** — fully scoped plan at `jlmops/plans/WOO_API_PUSH_PLAN.md` (2026-07-17, revised 2026-07-20). Its generator-duplication fix (item 1) shipped standalone @512 2026-07-19; category lookup, attribute-ID lookup, CSV export rework, and push service not started. Brand dropped from scope entirely (2026-07-20): no brand data exists today, and Google Shopping already maps Winery→Brand with high visibility, so a native Brand field closes no real gap; the former "Woo Brand + GTIN structured-data enrichment" item is retired, not folded in. Winery custom attribute stays in the push as a future hook. Sequencing open: this is catalog/ops infrastructure, not one of the acquisition-period's named channels — build now vs. queue behind acquisition work is an open call. Once built, extends to carry upsell/cross-sell values (separate later plan).
