...
4. **Next build — KPI Summary Tab, ahead of the open queue.** Approved 2026-07-02: compute `business/KPI.md`'s 6 defined KPIs (currently undefined anywhere) and wire them into `jlmops-status.md`'s existing KPI export — no jlmops UI. Plan → `jlmops/plans/KPI_SUMMARY_TAB.md`. After that: Reliability 1.3 or 4.1 → `jlmops/plans/RELIABILITY_AUDIT.md`; UI Tier 5 → `jlmops/plans/UI_AUDIT.md`; mobile LCP (~4.0s). (Content-workflow redesign and content-distribution plans are both fully shipped and archived — `jlmops/plans/_archive/`; facts live in `jlmops/docs/DATA_MODEL.md`.)
