...
4. **Meta Ads Jerusalem test — creative swap decided 2026-08-17, not yet built/live.** With bucket 8, bucket 14, and bucket 7 running roughly identically and a shared-ad-set budget diluted three ways (~200 clicks/₪100 actual spend over ~2 weeks vs. ~₪180-210 expected pace, no confirmed new-customer conversions), decision: drop bucket 8, keep bucket 14 (reviews) live, build bucket 18 ("Outsource It") as its replacement — net 2 ads, not 3. Remaining August budget to increase for the 2 survivors specifically. Test extended ~2-3 weeks past end of August (current window sits inside Israel's high-vacation period, a real confound; secondary branding-value rationale tracked via reach/frequency + branded-search/direct-traffic, not clicks/conversions) before pausing for the holiday season. Building bucket 18 is now the top production priority. See `marketing/plans/META_ADS_PLAN.md` Round Log for full reasoning.
5. **Google Search Ads test — built and launched 2026-08-17, new channel.** Text-ad (Responsive Search Ads) campaign, Jerusalem geo + loose English-language targeting, phrase-match keyword cluster ("wine online" / "order wine" / "wine shop online" / "wine delivery"), negatives set (cheap/fast/discount + others), Maximize Clicks bidding (Target CPA/Sales goal declined — same thin-conversion-volume reasoning as Meta), homepage as landing page (message-matches the site's own "You don't need to be an expert" headline, verified live). AI Max and audience-segment targeting both declined to keep the test controlled/legible. Currently in Google's Learning phase. Full spec not written up in `META_ADS_PLAN.md` per user request this session — reconstruct from Ads Manager or a future session if a fuller writeup is wanted.
...
8. **Galilee region post (Slot B, likely publishing 2026-08-16, see `content/plans/REGION_POSTS_PLAN.md`)** — in progress; drafted + registered in the library (`blog-region-galilee-en`, state `draft`). HE translation also already registered (`blog-region-galilee-he`, state `draft`) — a Library-tab display bug (fixed this session) had been hiding it from view. Companion email calendar row (`email-region-galilee`) moved to cal_Date 2026-08-18 (admin-edited directly in `JLMops_Publishing`, 2026-08-13).
9. **Central Mountains region post (Slot C) — body drafted through Image Prompts, 2026-07-09.** Calendar row staged (`blog-region-central-mountains`, cal_Date 2026-08-25, pending "Apply Pending Updates"); Drive doc placed at the canonical library path (`blog-region-central-mountains-en 26-07-09-12-05`); git source at `content/regions/central-mountains/central-mountains-en.post.md`. Remaining: winery verification (Gvaot/Tura not yet confirmed against JLM's carried wineries), Canva images, HE translation, library registration, WP push — same checklist as Galilee (`content/plans/REGION_POSTS_PLAN.md`). Companion email calendar row (`email-region-central-mountains`, cal_Date 2026-08-25) also staged 2026-08-10, pending merge.
10. **Grapes anchor post** ("Grape Varieties in Israel") — drafted through Image Prompts + Notes at `content/grapes/grapes-en.post.md`, facts verified. Not yet registered in the library or flagged to anyone. Individual grape spoke posts deferred per `content/plans/ISRAELI_WINE_GUIDE_PLAN.md`'s sequencing decision.
11. **Vintage-drift smoke test** — fix deployed @528 (`jlmops/plans/VINTAGE_MISMATCH_SNAPSHOT_FIX_PLAN.md`); needs a live vintage change to occur naturally before it can be confirmed working (no way to manufacture the test condition). Also: the task open since 2026-07-18 still carries the old, pre-fix stale snapshot — user to correct/clear it directly.
