...
**Updated:** 2026-06-07 — jlmops @257 · theme v1.2.29 live. BUNDLE_PLAN **Stages 0–2 COMPLETE; Stage 3 DATA/LOGIC COMPLETE** (@227→@245): serializer (`exportBundleWoosb`) + Export worklist (ops≠web **product multiset** diff) + qtyVariable & HE-text derived-from-web + out-of-stock failsafe + `getEligibleProducts` returns `wpm_ProfitRate`. **Stage 3 COMPLETE @256/@257** — profit-in-selector shipped (editor Phase 3); as-presented margin reads the WC WOOSB discount off the bundle's own `wpm_ID` row (fixed-amount/%/custom-price, user-confirmed fixed). **Found the discount was never imported** (REST pull captured woosb discount meta only on the HE-translation path, leaving `wpm_Woosb*` blank) → **@257 fixes the main-product pull**; needs one full API Pull to populate. Manual `sb_DiscountPrice` (final price) overrides as a test hook. Display/math only, never pushed (§7.1c). **`ADMIN_BUNDLES_UI_PLAN.md` Phase 1 (Queue & status) COMPLETE @245→@249** — 1a-i cached push status; 1a-ii `task.bundles.push_pending` singleton (housekeeping opens on count>0; **Export action closes it**) + dashboard "Bundles: Needs Push — N"; 1b needs-export filter chips (All/Needs export/OK) + status-sorted list (needs-export pinned, EN+HE muted line) + **alerts panel removed**; 1c **row-click editor in place** + deficiency strip. Also @249 **fixed editor price bar** (used backend `_calculateBundlePrice`; qty-0 no longer counted as 1 → no phantom savings). **Decisions this session:** parity dropped (jlmops-as-truth → EN/HE can't drift, self-correcting; §7.1a); dashboard Bundle Critical/Low rows removed, Needs Push is the sole bundle signal (§7.1b); **discount is WC-managed/manual — jlmops never pushes it** (§7.1c). Current diff 0 of 14 (Phase 1 + 2 un-exercisable live until a bundle change makes count>0). **Phase 2 (Export UX) SHIPPED @250** — Export button now generates the worklist as an auto-opened Google Sheet (Bundle · EN · HE · Warnings, via the product-detail export sequence → `system.folder.jlmops_exports`); task auto-close moved to the real export action (`WebAppBundles_exportBundlesToSheet`). **Phase 3 SHIPPED @251 (smoke pending):** the AdminBundles editor is now the §7.4/§7.5 composition sheet — header strip (name EN/HE, status, type badge, read-only price summary) + one ordered list (text headers; product rows w/ ↑↓ reorder, qty stepper + qty-0 Flexible pill, name/SKU/price/profit, full criteria behind ▸) + client draft/Undo + atomic `saveComposition` + product-picker ModalOverlay + sessionStorage/beforeunload, replacing the two-pane editor. Backend enablers (data contract): `getBundleWithSlots` enriches slots w/ name/price/profit; `getEligibleProducts` accepts `draftSlot`+`excludeSKUs`; `createSlot` id collision-proofed; save blocks empty-SKU product rows. **Next = SMOKE-TEST @256** (live, no /dev; editor UX iterated @252→@255: collapsed product row read-only data, editing incl. Replace/qty/Flexible in a 'Details' disclosure; @256 as-presented margin now reads the real WOOSB discount). Then Stage 4 parity cleanup + 5→6→7 intelligence arc., then Phase 4 (Stage 7 deficiency) + BUNDLE_PLAN Stages 4–7. **Standing jlmops backlog:** above + reliability-audit + UI-audit queues; `TEST_HARNESS_PLAN.md`; CODE_AUDIT Phase 1 authz; RELOAD_RESILIENCE Option A. (Narrative → `.claude/session-log.md`.)
...
| Revenue | Steady |
| Deploy Version | jlmops deploy @257 · theme v1.2.29 LIVE |
| Deploy Date | jlmops 2026-06-07 (deploy @257) · theme 2026-06-04 (v1.2.29) |
...
- **Bundle handling — `jlmops/plans/BUNDLE_PLAN.md`** (consolidated staged master plan). **Stages 0–2 COMPLETE + Stage 3 CORE shipped @227→@242** (re-sequenced numbering): **0** qty=0 price fix; **1** Bundles-view perf; **2** cost/profit data layer; **3 core** = serializer (`exportBundleWoosb`, tokens reused from slotId) + Export worklist on AdminBundles (ops≠web **product-multiset** diff — token/order/text-agnostic) + qtyVariable & HE-text now derived from web — verified live. **Stage 3 remaining:** profit-in-selector, as-presented profit, out-of-stock failsafe. Then **4** EN/HE parity + stock (mostly built); **5** fast bundles-only member refresh; **6** cross-bundle diversity; **7** unified suggestion generator. **AdminBundles UI overhaul → `ADMIN_BUNDLES_UI_PLAN.md` (Dispatch-led).** REST push parked. Implement depth per-stage.
