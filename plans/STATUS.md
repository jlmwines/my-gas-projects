...
- **Newsletter v1** — printed monthly insert (online + store handout) + companion email. Layout + workflow refined 2026-05-19 (`marketing/NEWSLETTER_PLAN.md`): A4 b/w, masthead → recurring B&W vine-to-glass banner → two-col body → footer; pandoc + reference docx templates (EN + HE) build the layout, Canva role narrowed to image-gen only, Drive MCP delivers finished docx into a known folder. **"Making Wine — [topic]" series** is the recurring subscriber-exclusive monthly journal of vineyard/winery activity (12-month draft already exists, Evyatar edits month-by-month); lives ONLY in print newsletter + companion email, NOT on the blog. Email opens with the Making Wine content (no series-launch announcement), secondary mentions below. Site footer signup CTA copy gets updated alongside Issue #1 to the "follow Evyatar / behind-the-scenes" framing. **2026-05-19: Context post LIVE** — EN https://jlmwines.com/context/ and HE https://jlmwines.com/he/context/ published with featured image, 4 body images, cross-links to pairing/intensity/acidity/complexity posts, mobile-first column stacking (cols-flip). Remaining for Issue #1: reference docx templates EN+HE (one-time setup), SVG signature placeholders, recurring banner art, Mailchimp content paste (subject/preview/HTML body — drafted in chat, pasted manually because Mailchimp MCP can't edit user-created drafts).
