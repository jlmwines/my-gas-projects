...
- Phase 5 (task UI rebuild covering `ManagerDashboardView_v2` + `AdminDashboardView_v2` surfaces) is **built as a parallel new view, not an in-place edit**. New file `ManagerDashboardView_v3` (and admin counterpart) lives alongside the live v2; current daily-use code is untouched. The v3 view is reachable via a separate nav link (or `library.enabled` flag) for testing. Promotion is a one-line nav-route swap at the end of the soak period. Worth its own plan doc.
...
5. **Library UI list view + task UI parallel build** (reordered 2026-05-25 — was phase 4; revised 2026-05-25 — parallel new view instead of in-place refactor). Build read-only library list using the common-skeleton + type-pack pattern with a shared widget kit (§11). **Build new `ManagerDashboardView_v3` (and admin counterpart `AdminDashboardView_v3` / unified queue) as parallel new files; do NOT edit v2 in place.** v3 covers the formalized task pattern + project-as-filter (collapses `AdminProjectsView` into a queue filter). v3 reaches via a separate nav entry or behind the `library.enabled` flag — current v2 remains the live default. Both views read the same SysTasks rows (and the new polymorphic `entity_type`/`entity_id` columns from §7). Promotion = one-line nav-route swap once soaked. Own plan doc (`jlmops/plans/LIBRARY_TASK_UI_REFACTOR_PLAN.md`, to be drafted in chavruta as the first move of this phase). Validates the type-pack shape against real entity data created in phase 4. **Risk profile collapses to additive** — daily-use code untouched, swap is reversible.
