...
- **Library file = Claude's read surface;** no general-purpose export pipe. **Publishing stays external** (blog via `content/scripts/push-posts.js`; Mailchimp/social/WhatsApp/video manual; "publish" tasks close as confirmations recording the external URL). **Publishing calendar** — superseded by the later "calendar is king" decision (`jlmops/docs/DATA_MODEL.md` "Publishing Calendar", `CALENDAR_LIBRARY_LOOP_PLAN.md`, 2026-07-08): the live `JLMops_Publishing` sheet is the calendar of record, not a markdown mirror.
