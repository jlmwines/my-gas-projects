...
- **Mobile.** Desktop-only for v1 (right-side slide-in panel over LibraryView, `modal-overlay` pattern, full-height). Phone takeover modal deferred until a real mobile drawer use case surfaces. Matches the "shared widgets responsive once; per-pack cost when needed" principle below in Mobile.
