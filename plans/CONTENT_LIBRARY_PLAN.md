...
2. **Scope is smaller than it first looked.** A pre-implementation code audit found most of the supposed "build" already exists: the normalized `_getQueueTasks` feed (complete shape incl. `packForm`), full-field `WebAppTasks_updateTask`, the `st_ProjectId` filter (`getFilteredTasks`), and the entity→task task-id payload. **No SysConfig JSON edits.** Net effort is a medium, front-end-heavy refactor (~3–4 incremental sessions), additive and reversible (fallback preserved), low architectural risk — dominant cost is careful UI assembly + per-pack verification, not new capability. Genuinely new work: extract `packBody()` into a shared include, assemble the new view, wire the drawer-row `onclick`, and (only for truly free-form tasks) relax `WebAppTasks_createTask`'s mandatory `projectId`.
