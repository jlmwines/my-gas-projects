...
| GSC multi-tab wiring gap | **Fully resolved, confirmed live @427.** Root cause was compounding: (1) `data_tab` config held the workbook's title, not a real tab name; (2) the underlying assumption that GSC needed a Date dimension was itself wrong — the documented setup guide always specified Page-grouping only (GSC is a diagnostic tool per `KPI.md`, not a trend metric; GA4 carries trend). Reworked `StatusReportService._readGsc` to report total clicks/impressions + top pages by clicks, with week-over-week trend derived from a SysConfig snapshot (`system.kpi.gsc_last_snapshot`) instead of a Date column. Verified live: 2,140 clicks / 117,221 impr / avg pos 9.5, first snapshot recorded. Surfaced a real (non-urgent) SEO finding along the way — homepage tracked as two separate http/https URLs — logged in `.claude/bugs.md`. |
