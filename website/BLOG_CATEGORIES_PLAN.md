...
3. **Not yet — hold until the first region post is ready to publish.** At that point, in one deliberate pass: build the `inc/sections.php` multi-category render path (tab row + `category__in` filtering + per-language term resolution), the `All` view (category-label badges per card), and wire `template-articles.php` to it. Tabs for individual categories stay data-driven (only show a tab for a category with ≥1 published post); `All` is just written in as a normal tab at this point, not behind any runtime "2+ categories populated" check — by construction it's only being built once that's already true.
4. **`content/push-posts.js`** — add `enCategoryId: 1272`/`heCategoryId: 1273` to the first region post's manifest entry when it's ready to push, so its category is set automatically (mechanism already exists in the script per `content/PUBLISHING.md`). This is what makes step 3 timely to do — do it alongside preparing that first push, not before.
...
Drafted 2026-07-06 (revised twice same day: filter-tabs approach, then post-count-gated per-category tabs, then deferred `All` + the whole tab-row build to when the first region post is ready to publish, rather than building it now against a category with 0 posts). Categories created live (steps 1-2 done). Steps 3-5 (all the code) intentionally not started — this doc is the placeholder for that work, to pick up when Negev (or whichever region post) is ready to go live.
