...
- [ ] 2026-07-12: **Manager product-detail submit sometimes never reaches the server — button stuck on "Submitting…" indefinitely, nothing written.** Recurred 2026-07-13 on a new-product (`task.onboarding.add_product`) submission, confirming it isn't scoped to one task type or SKU — both share the same `submitProductDetails` code path. `SysLog` shows zero trace of the failed attempt both times, consistent with the click never reaching the server. Root cause not yet found; next step is a live repro with browser DevTools console open. Analysis → `jlmops/plans/BUG_FIX_SEQUENCE.md` Session J.
