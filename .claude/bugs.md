...
- [ ] 2026-07-01: **Homepage tracked as two separate URLs in Search Console** — GSC's page-level report shows `https://jlmwines.com/` and `http://jlmwines.com/` as distinct rows with separate click/impression/position histories (148 clicks/pos 9.4 vs 49 clicks/pos 6.5 in the first `jlmops-status.md` GSC snapshot). Suggests the http→https redirect/canonicalization for the bare homepage isn't fully clean — likely related to the mixed-content HTTP image bug below. Not urgent; worth folding into the next SEO cleanup pass.
