...
- [ ] 2026-07-24: Admin Dashboard's Projects panel (`_getProjectSummaries`) calls `ProjectService.getAllProjects()` with no status filter — includes `COMPLETED`/`ARCHIVED` projects alongside `ACTIVE`/`PLANNING`, cluttering the view with non-central projects. `ProjectService.getProjectsWithStatus()` already exists as the fix precedent. Deferred alongside the Tasks card item above — same panel, same redesign conversation.
