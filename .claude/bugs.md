...
- [ ] 2026-07-12: **Manager product-editor loads take 15-18s.** Original cause (CacheService failing 100% of the time on WebDetM/CmxProdM/WebProdM, >100KB-per-value limit) fixed @469 — but 2026-07-13 smoke test found total open time unchanged even with confirmed cache hits, so the real bottleneck is still open. Analysis → `jlmops/plans/BUG_FIX_SEQUENCE.md` Session J.

- [ ] 2026-07-12: **Manager submit/verify modal gets stuck on "Submitting…"/spinner, image and fields silently swap to a different product.** Root cause found 2026-07-14, not the original "never reaches server" framing: `openEditor()` (`ManagerProductsView.html`) had no guard against being called again while a submit/verify/revert call was still in flight (both take 6-8s server-side) — a second task load mid-submit overwrote the shared modal state without ever resetting the original button. **Fix shipped @479** — in-flight flag blocks `openEditor()` re-entry, plus a defensive button-state reset. Live smoke-test found a second, distinct race (see next entry) — @479 itself not yet independently confirmed clean. Analysis → `jlmops/plans/BUG_FIX_SEQUENCE.md` Session J.

- [ ] 2026-07-14: **Opening the next verification task shows stale data from the previous task; new-product onboarding modal shows leftover "current data" that should be blank.** Found smoke-testing @479, fix @480 (`_loadToken` race guard) didn't resolve it — symptom was consistent, not intermittent, pointing at a structural gap instead: `loadVerifyDetail()` only clears the verify-mode display panel (image/category/flags) after the slow main load resolves, nothing clears it synchronously on open. **Fix shipped @481** — moved that clearing into `openEditor()`'s synchronous block. Same deploy also restored verification-submit auto-advance to the next task (`confirmVerify()` → `advanceVerify()`, button relabeled "Done & Next" to avoid the exact label/behavior mismatch @208 fixed by removing this once before) — the forced manual reopen was itself making the loading-window gap easy to hit. Not yet smoke-tested live. Analysis → `jlmops/plans/BUG_FIX_SEQUENCE.md` Session J.

- [ ] 2026-07-14: **Admin Products "Verify" needs a card showing tasks assigned to the Manager but not yet completed.** No such card exists today; add one surfacing open (incomplete) Manager-assigned tasks on the verify view. Enhancement, not a defect.
