...
- [x] 2026-07-26: `WebXltM` found empty, blocking every sync — Session P's column-remap fix (@530-543) matched staging/master headers by exact name, but `WebXltS`/`WebXltM` intentionally use different prefixes (`wxs_`/`wxm_`), so every row wrote blank. Fixed via existing `map.staging_to_master.web_translations` config mapping, deployed @544 (`27bd2cd`). Related gaps (empty-master hard-crash, multi-phase pull atomicity) logged as open in `jlmops/plans/RELIABILITY_AUDIT.md` §1.5/§1.6.
