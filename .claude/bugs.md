...
- [ ] 2026-05-04: **CRM cleanup** (bundle). Reconciled with `jlmops/plans/CRM_PLAN.md` simplified rule (2026-04-30 revision) on 2026-05-13. The simplified rule supersedes the older 12-step plan and changes what's actually on the table:
  ...
  - **Archive mapping missing `CouponItems` field** — **RESOLVED.** Verified 2026-05-28 Session A: `ContactImportService.js:99` already has `mappedRow[womIdx['wom_CouponItems']] = row[womaIdx['woma_CouponItems']];`. Fix matches CRM_PLAN.md §380 exactly. Bug entry was stale — fix had been applied but never marked done.
  ...
  Net remaining work: 2 items (wire gift rule, run correction script) + 1 optional housekeeping (delete war-support detection code).
...
- [ ] 2026-05-15 (re-confirmed 2026-05-27): **Admin Projects task delete — claims completion but leaves records.** jlmops admin Projects: deleting selected tasks often returns a result saying "failure to delete 1 or more selected records" (or similar partial-success message); the selected rows remain in the list afterwards. Display also lags. Not yet diagnosed. Likely candidates: race between client refresh and server write; per-record failures (e.g., archived/locked rows) bubbling to top-level message without row-level reporting; selection set not matching what the backend deletes.
...
- [ ] 2026-05-27: **SKU Replacement leaves orphans on the web side.** When a SKU has been changed in Comax but is still active in web, running the SKU Replacement action should rewrite the old SKU everywhere it lives on the web side: `WebProdM` (product master), `WebProdM_Staging`, `WebDetM` (details master), `WebDetM_Staging`, and `st_LinkedEntityId` on any open `SysTasks` row referencing the old SKU. Observed behavior: the replace action fails to locate the old SKU on the web side (suggests it's only scanning Comax, not querying web product files) and leaves orphan references behind. Result: the web side continues to point at the old SKU after the action reports complete. Diagnosis needed: trace which sheet(s) `WebAppProducts` / `ProductService` actually rewrite during a replace; confirm whether the lookup path uses `wpm_SKU` / `wdm_SKU` queries at all, or relies solely on Comax-side matching.
