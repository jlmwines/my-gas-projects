...
- [ ] 2026-07-24: **Correction to the entry below (was wrong about which screen):** Admin Dashboard has no Projects panel — confirmed removed deliberately at `94d4953` (@485), before this session; not a bug. `ManagerDashboardView_v2.html` does still render a live Projects card (`renderProjectsCard`, fed by the same unfiltered `_getProjectSummaries`/`ProjectService.getAllProjects()`, so the no-status-filter issue is real there, not on Admin). Owner's view (2026-07-24): projects aren't directly manipulated by either role day-to-day — they exist mostly as an internal grouping (Distribution Projects auto-tied to content/campaign chains), not a first-class list users browse. Downgraded — low priority, not worth a redesign session; revisit only if it becomes a real point of confusion.
...
- [ ] 2026-07-23: **`doGet`'s role check is a login gate, not an authorization boundary** — it only blocks unlisted "viewer" users; any listed user of any role gets the identical full SPA shell, `availableRoles` lists every role regardless of caller, and the role-switcher dropdown is 100% client-side (no server round-trip). A manager can click "Admin" in the UI and load real Admin screens + every mutating `google.script.run` endpoint elsewhere in the codebase (Product/Inventory, CRM/Campaigns, Library/Project — no server-side re-check exists anywhere). `jlmops/plans/CODE_AUDIT_PLAN.md` §9 (WebApp controllers pass) — supersedes the per-subsystem authz entries below, which are the same root cause found repeatedly.
- [ ] 2026-07-23: `sc_DoNotContact` field referenced by 4 marketing-export functions doesn't exist in the SysContacts schema — do-not-contact suppression has never actually excluded anyone from these exports. `jlmops/plans/CODE_AUDIT_PLAN.md` §9 (Phase 3).
- [ ] 2026-07-23: Campaign Detail's project list always empty — `WebAppCampaigns_getCampaignDetail` filters on `spro_CampaignId`, dropped from SysProjects when the Campaign↔Project FK direction reversed. `jlmops/plans/CODE_AUDIT_PLAN.md` §9 (Phase 3).
- [ ] 2026-07-23: `LibraryService.js` has no `LockService` around check-then-write entity/file creation — double-click or concurrent sessions can create duplicate `SysLibrary` rows or duplicate Drive Docs for the same slug. `jlmops/plans/CODE_AUDIT_PLAN.md` §9 (Content Library pass).
- [ ] 2026-07-23: `WebAppDashboardV2._getOrdersData_v2` reads nonexistent fields `sol_Status`/`sol_ComaxExported` (real: `sol_OrderStatus`/`sol_ComaxExportStatus`) — every dashboard load shows onHold/processing/newOrders/ordersToExport as 0 regardless of actual counts. `jlmops/plans/CODE_AUDIT_PLAN.md` §9 (WebApp controllers pass).
- [ ] 2026-07-23: `AdminTasksView`/`AdminProjectsView`/`AdminBundlesView` bypass the mandatory `ModalOverlay` controller (raw `style.display` toggling) — no focus trap, Esc-close, scroll-lock, or z-index coordination on their modals. `jlmops/plans/CODE_AUDIT_PLAN.md` §9 (Admin UI pass).
- [ ] 2026-07-23: `AdminProductsView.html` never uses `TaskWidgets.escape` — product names render unescaped via `innerHTML` in 4 search-result renderers; a name containing `<`/`"` breaks the row. `jlmops/plans/CODE_AUDIT_PLAN.md` §9 (Admin UI pass).
- [ ] 2026-07-23: `AdminInventoryView.html`/`AdminDashboardV2.html`/`AdminContactsView.html` inconsistently escape task titles/notes/contact email in `innerHTML`/data-attributes — an admin-typed task title or a contact email with `<`/`"` can inject on Inventory/Dashboard/Contacts views. `jlmops/plans/CODE_AUDIT_PLAN.md` §9 (Admin UI pass).
- [ ] 2026-07-23: `OrdersView.html` renders customer order notes (genuine customer-supplied WooCommerce checkout text) unescaped into a table cell and an inline `onclick` attribute — a quote breaks the Gift Doc button, script markup executes. `jlmops/plans/CODE_AUDIT_PLAN.md` §9 (Manager UI pass).
- [ ] 2026-07-23: `TaskDetail.html`'s generic "Done" button renders for content tasks too — closing a content task this way skips `WebAppLibrary_lockVersion`, leaving the entity never versioned/locked. `TaskPacks.html`'s `hasTypedPack()` exists to prevent this but is never called. `jlmops/plans/CODE_AUDIT_PLAN.md` §9 (Manager UI pass).
- [ ] 2026-07-23: `ManagerProductsView.html` never uses `TaskWidgets.escape` — product/task names render unescaped via `innerHTML` in 4 renderers, same defect class as `AdminProductsView.html`. `jlmops/plans/CODE_AUDIT_PLAN.md` §9 (Manager UI pass).
- [ ] 2026-07-23: `ManagerContactView.currentDirection()` queries a `mc-direction` radio group that doesn't exist in the markup — every "Make contact" attempt logs as Outbound regardless of actual direction. `jlmops/plans/CODE_AUDIT_PLAN.md` §9 (Manager UI pass).
- [ ] 2026-07-23: `TaskService.updateTaskDates` calls nonexistent `_invalidateWebAppTasksCache()` — throws after the sheet write succeeds, so task date edits report failure to the manager even though they persisted. `jlmops/plans/CODE_AUDIT_PLAN.md` §9 (Core Plumbing pass).
- [ ] 2026-07-23: `HousekeepingService.refreshCrmContacts`/`checkBundleHealth` gate their skip-optimization on nonexistent `syncSession.currentStage` field — always false, so the full CRM refresh + bundle scan runs unconditionally every daily cycle instead of only after a sync. `jlmops/plans/CODE_AUDIT_PLAN.md` §9 (Core Plumbing pass).
- [ ] 2026-07-23: Woo order API-pull hardcodes 24-line-item cap instead of reading `web.order.line_item_schema.max_line_items` — diverges from the CSV order path if that config value changes. `jlmops/plans/CODE_AUDIT_PLAN.md` §9 #1.
