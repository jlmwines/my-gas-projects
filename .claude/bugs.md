...
- [ ] 2026-08-27: A `queue.failed_job_sweep` task shows `st_Status='Done'` with a done date set, but still appears on the dashboard as open. Same shape as the known `st_DoneDate`-without-`Done`-status pattern in `plans/STATUS.md` Current State, but here status itself already reads Done — a display/query gap, not a write-path gap. Not yet traced. `jlmops/plans/SYSTEM_TASK_LIFECYCLE_PLAN.md` is the likely home once investigated.
