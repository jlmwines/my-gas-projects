...
- [ ] 2026-07-12: **Manager product-editor loads take 15-18s because CacheService caching for WebDetM/CmxProdM/WebProdM fails on every call.** `ProductService.getProductDetails`'s `getRowObject` helper (`ProductService.js:1073-1170`) builds the entire sheet as a Map and JSON-stringifies it for `CacheService.getScriptCache().put()` — but WebDetM, CmxProdM, and WebProdM all exceed CacheService's 100KB-per-value limit, so `cache.put()` throws `Argument too large: value` on every single call (confirmed in SysLog: 100% miss rate across every sampled editor open). Only WebDetS (small staging table, 7-9 rows) actually caches successfully. Result: every product-editor open does 4 full uncached `getDataRange().getValues()` reads, taking 15-18 seconds total (confirmed: 16076ms, 18389ms, 14941ms, 16173ms across 4 separate opens the same day). Fix plan → `jlmops/plans/BUG_FIX_SEQUENCE.md` Session J.

- [ ] 2026-07-12: **Manager product-detail submit sometimes never reaches the server — button stuck on "Submitting…" indefinitely, nothing written.** Confirmed via manager screenshot + SysLog cross-check for SKU 7290101582403: the submit click set the button to "Submitting…" client-side (`ManagerProductsView.html:1809`) and it stayed there for several minutes, but `ProductService.submitProductDetails`'s very first log line (before it touches any data) never appears anywhere in SysLog for that SKU, across 3 separate editor opens in the same session. Submits for two *other* SKUs succeeded normally in the same window (18:12:52, 18:29:23), ruling out server-side quota/load as the cause — the request appears to never leave the client for this specific click. No `WebDetS` staging row was written for the stuck SKU. Root cause not yet found; possibly related to/exacerbated by the cache bug above (longer modal dwell time = more exposure to a mobile connection drop or stale event binding from repeated reopens) but not confirmed as the direct cause. Fix plan → `jlmops/plans/BUG_FIX_SEQUENCE.md` Session J.

- [ ] 2026-07-10: **`jlmops-status.md`'s "Failed jobs" health metric reports only the oldest failure's age, not the newest — can't signal a job that just failed.** `HousekeepingService.js:778-779` computes `failed_job_count`/`failed_job_oldest_age_days` from `failedJobsSummary` once per daily housekeeping run (`Last run: ... 02:35`) — no `newest`/`youngest` field exists, and the whole block is a snapshot, not live, so a job that fails mid-day won't surface here until the next day's run. Found while checking whether the daily-review Pulse check could miss a just-happened failure — the only near-real-time signal is the separate "Recent errors" block (`LoggerService.getRecentErrors`, ~15-min refresh, scans `SysLog` tail for ERROR-level rows), which only catches failures whose path actually writes to `SysLog`. Fix shape: add `failed_job_newest_age_days` (and ideally a timestamp) alongside `oldest` in `HousekeepingService`'s failed-jobs summary and `StatusReportService._dataQualityBlock`, so a fresh failure is visible same-day without depending on Recent-errors' SysLog write path.
