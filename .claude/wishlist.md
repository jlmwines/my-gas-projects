...
- [ ] 2026-06-10: **Loading/loaded indicator in the view title bar.** A consistent per-view load-state indicator in the title bar (spinner while the view/its sections load, settled state when done) so the operator always knows whether data is still arriving. Complements the per-view lazy-load work (Admin Products/Inventory/Bundles @273+); today load state is ad-hoc per card. Reaffirmed 2026-07-16: without it, a blank card is ambiguous between "still loading" and "genuinely no data" — no way to tell at a glance. Placement preference (2026-07-16): single indicator for the whole view — either in the main header near the view name, or a bottom-of-screen indicator — that changes or disappears once every section has finished loading.
