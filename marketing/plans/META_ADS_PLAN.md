...
**Status.** Not started. No campaign built yet. Business Manager access confirmed. Meta Pixel not currently enabled but previously used via a plugin and re-enablable if wanted (see Prerequisites #3) — current plan is to run on UTM + WooCommerce order-attribution data instead.
