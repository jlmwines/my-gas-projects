...
**Doorbell-cam asset, build note (2026-07-30):** no real doorbell app or device is used or needed — fully open to build as wanted. **No identifiable faces (no courier in frame) and no identifiable address/house number** either way — closes both the trademark-exposure risk (a real app's branding) and the privacy/consent risk (an unconsented person or a real address) an actual doorbell-app screenshot would have created. Two visual routes, not yet chosen between:
- **Static image, fisheye crop only, no notification banner.** A staged photo of a package at a door (any regular phone camera), circular/fisheye crop for the "doorbell cam" look. Safer route against the Meta system-UI-mimicry risk flagged above, since nothing simulates an actual OS/app element — it's just a photographic framing choice.
- **Notification appearing on a phone screen.** More "phone-native" feeling, consistent with the piece's whole phone-screen-recording conceit, but still carries some of the same system-UI-mimicry risk the red-team flagged, since a lock-screen notification is itself a standardized OS element being imitated (a different specific banner than the original "Package Delivered" idea, same underlying category of concern).
...
**Box asset, build note (2026-07-30, updated).** Doesn't need to be a real branded carton, and now genuinely can't be one anyway — `plans/STATUS.md` records that the original custom-printed-carton plan was canceled (shipping cost ran way over the cartons' own cost), replaced with **natural brown cartons + a large logo stamp**. Convenient overlap: that's essentially the same treatment this ad asset needs. Build it digitally: a plain natural-brown cardboard box (any stock photo or Canva element) with the JLM mark added as a clipart/mockup overlay, sized like a large stamp on its side — matches where the real packaging is actually headed, not just an ad-only mockup. **Stock video option, worthwhile with one condition:** if Canva's stock video library has a suitable **static, locked-off** clip of a plain box at a door (camera not moving, box stationary, no competing branding, no visible address), that's an even lower-cost route than a generated/staged still — no custom generation step, just search and drop in, then overlay the logo graphic on top. Only works if the shot is genuinely locked-off, though — any camera motion and a fixed-position logo overlay won't track to the box's surface, reading as a floating sticker instead of something printed on it.

**Production risks (red-team pass, 2026-07-30) — needs resolving before any production spend:**
- **Meta ad-policy risk, treat as seriously as bucket 1's gambling flag.** The "Package Delivered" notification banner and the tap-cursor indicator on the "order wine" entry both resemble what Meta's ad-review policy against mimicking system/OS UI and implying non-functional interactivity targets. Likely needs restyling — a more clearly illustrated/stylized banner and tap graphic, less literal notification-mimicry — not necessarily dropping the mechanic, but a policy check first.
...
- **Tap-indicator graphic has no assigned build source yet** — needs one (a simple cursor/hand icon from Elements, or a drawn ripple shape), not left implicit.
