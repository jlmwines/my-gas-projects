...
**8. Calendar of confidence.** Built jointly across a long back-and-forth (2026-07-30) — better than either the session or the user's team-review passes produced alone, so credited as a joint concept, not a session output. One continuous frame, not cut-away scenes: a full-month calendar is the whole canvas, and each recurring wine-decision moment shows up as a day getting a brief zoom/projection "callout" — a pictogram for that day's occasion pulled forward, held a beat, released back into the grid. Six occasions populate the month, each a language-agnostic pictogram (no baked-in text, callback to the Prompt Formula rule above): **personal dining** (cooking island), **hosting** (hot coals + food ready to grill — BBQ, culturally load-bearing in Israel), **Shabbat** (table set — deliberately separate from hosting; Friday evening is a family ritual that stands alone, not a hosting sub-case), **attending an occasion** (a door with balloons — signals "there's a party here" without needing a person), **gifting** (a calendar day with a birthday marked — reframed from "buy a gift" to "choosing which bottle you already have suits this person," tying to bucket 6's buy-ahead stock and to product-attribute transparency: knowing a bottle's intensity, for instance, makes it a safer pick for an unknown palate), and **personal enjoyment** (a cozy reading nook — book, couch or recliner). **Design rule: no bottle ever appears on a day's callout** — only a question-mark-style pictogram. The box/bottle motif lives in its own corner, pulsing or glowing in sync with each callout, so the single reveal stays the only place a bottle appears in the whole piece — keeps the "felt, not stated" answer consistent and makes the one payoff visual do all the work. **Timeline anchor (resolves a real coherence problem — a month of "already handled" days can't also be the moment a NEW customer is invited in):** the shown month starts at an order/delivery date (the rack fills, echoing bucket 6's fill motion) — it's someone else's already-lived month, not the viewer's. **Visual treatment (2026-07-30, joint, revised same day).** The whole Reel is framed as a phone-screen recording throughout, not just an opening shot with a phone as a prop. Opens on a **doorbell-cam app view** showing the delivery arriving — the "order lands" moment happens natively inside a phone screen rather than needing a separate physical unboxing scene at all. From there, switching to the calendar app is just moving between two things already on the same phone — no extra motivation needed, since the piece has been phone-screen framed from its first frame. Gets the icon legibility a day-callout needs, and matches the screen-recording aesthetic buckets 9/10 and the help-a-friend concept already use (native/authentic register, per the Format & Angle Library's UGC note). **Breakout mechanic, resolved (2026-07-30, joint):** as the page turns to the new month, the sequence lands on the actual site — jlmwines.com, with the already-live WhatsApp button/contact visible in that shot (WhatsApp Business is genuinely set up and site-linked, per `.claude/wishlist.md`; the floating WhatsApp button shipped in the 2026-05-03/04 theme deploys per `CALENDAR.md`) — then the phone-screen chrome falls away, expanding to full-bleed for the first time in the piece. This grounds "you already know which guy" in something literally true rather than a metaphor (a real, already-available direct line to Evyatar), and gives the breakout somewhere real to land instead of an invented interface. Supersedes an earlier incoming-call alternative — no invented device needed once the destination is the real site. **Evyatar's arc carries the turn:** small, constant, and still in one corner throughout that month — the one thing that doesn't move while days flip past — then, as the page turns to a new, blank month, he grows and steps into center frame, addressing the viewer directly for the first time in the piece. **Payoff structure:** the everyday-savings message stays quiet background texture during the small-Evyatar section; the overt new-customer offer (₪50 off, code `50NEW`) appears only in the centered, direct-address close, so it gets the one deliberately built impact moment instead of competing with everything else. Closing line, spoken by centered Evyatar: an invitation to visit the site, save, and enjoy with confidence. **Closing beat sequence (2026-07-30, joint):** candidate copy for the centering climax, one short line per beat, each closing a thread the piece spent its runtime opening — *"You already have the app."* (bucket 4's refrain, flattened from question to statement) / *"You already know which guy."* (names the advisor thread as already-known, not new information) / *"You're lucky."* (reclaims bucket 1's gambling/luck language — same word, flipped from warning to already-won) / *"And clever."* (status close, same register as the cool-kids/advisor-status thread) / *"You have the solution in your hands"* (synced to the box/bottle reveal — literal and metaphorical on the same beat). **Adjacent idea, not folded in:** an "empty rack, trigger replenish" ending using the same visual language reads as a retention/reorder nudge for existing customers rather than new-customer acquisition — a natural sequel piece once there's a customer base to retarget, not this bucket's scope.
