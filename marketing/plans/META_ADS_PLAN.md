...
- **This project (JLM Wines sessions)** — the strategy/creative side: develops the creative candidate library (Creative Strategy below), writes the Canva prompts for each asset (same mechanism already used for blog/newsletter images — a session crafts the prompt, it never produces the asset itself), works out the test structure as rounds actually happen (method still open — see Testing approach below), maintains round results and next-round decisions locally in this doc (see Round Log below), and reviews exported performance data.
