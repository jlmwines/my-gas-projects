...
**Doorbell-cam asset, build note (2026-07-30):** no real doorbell app or device is used or needed. Build it as a generic illusion: an ordinary staged photo of a doorstep with a package (any regular phone camera) run through a circular/fisheye crop + a generic "Package Delivered" banner made from Canva's own shape/text elements — not a screenshot of a real app's UI. **No identifiable faces (no courier in frame) and no identifiable address/house number** in the staged photo — closes both the trademark-exposure risk (a real app's branding) and the privacy/consent risk (an unconsented person or a real address) an actual doorbell-app screenshot would have created.
